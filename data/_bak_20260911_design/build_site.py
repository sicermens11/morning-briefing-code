...
/* ── 표지 ── */
#home{justify-content:center;align-items:center;text-align:center;gap:0;padding:40px 20px}
...
#home .eyebrow{font-family:__MONO__;font-size:14px;letter-spacing:.24em;color:#6b665c}
#home h1{font-size:clamp(38px,9vw,64px);font-weight:800;letter-spacing:-.045em;
  color:#1c1813;margin:18px 0 0;line-height:1.08;text-wrap:balance}
#home .sub{font-size:clamp(15px,3.6vw,19px);color:#6b665c;margin:14px 0 0}
#home .when{font-family:__MONO__;font-size:14px;color:#9d7a17;font-weight:700;
  letter-spacing:.1em;margin-top:34px}
#home .btns{display:flex;flex-direction:column;gap:12px;margin-top:20px;width:100%;
  max-width:330px}
.big{display:block;width:100%;box-sizing:border-box;padding:18px 20px;border-radius:12px;
  font-family:inherit;letter-spacing:-.01em;cursor:pointer;
  border:1px solid #d5cec0;background:#e8e4dc;color:#1c1813;transition:.15s;text-align:left;
  display:flex;align-items:center;justify-content:space-between;gap:10px;
  /* ⭐ **네 상자 크기를 고정한다** (2026-09-09 사용자 지정).
     글자가 길어도 상자는 안 커진다 — 반드시 **두 줄**로 끝난다 */
  height:78px}
.big:hover{border-color:#1b3bf0;color:#1b3bf0}
/* 안쪽은 두 줄: ① 제목 + 갱신일시  ② 부제 */
.big > .lab{flex:1;min-width:0;display:block}
.big .t1{display:flex;align-items:baseline;justify-content:space-between;gap:10px}
.big .t1 b{font-weight:700;font-size:17px;line-height:1.15;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
/* ⭐ 갱신일시와 부제는 **색은 같게**, 부제만 **2pt 작게** (2026-09-09) */
.big .up,.big .sub{font-style:normal;font-weight:400;line-height:1.3;
  color:#8a8378;opacity:1;white-space:nowrap}
.big .up{font-size:12px;flex:none}
/* ⭐ 제목과 부제 간격을 좀 더 붙인다 (2026-09-09) */
.big .sub{font-size:9px;display:block;margin-top:1px;
  overflow:hidden;text-overflow:ellipsis}
.big .arw2{font-size:19px;opacity:.5;flex:none}
#home .note{font-size:12px;color:#9c968c;margin-top:28px;max-width:330px;line-height:1.6}
...
def quant_view(q):
    ...
    위1 = (f'<div style="font-family:{MONO};font-size:26px;color:{C["sub"]};'
           f'letter-spacing:.18em;font-weight:700">QUANT · 숫자로만 고른 종목'
           + (f' · {잰때} 확정' if 확정 else '') + '</div>'
    ...
    if not 후보:
        ...
        가1.append(
            f'<div style="display:flex;flex-direction:column;'
            f'justify-content:center;height:100%;text-align:center">'
            f'<div style="font-size:88px;font-weight:800;'
            f'letter-spacing:-.04em;line-height:1.1">오늘 볼 것<br>없음</div>'
            f'<div style="font-size:30px;color:{C["faint"]};margin-top:28px;'
            f'line-height:1.7">조건을 통과한 종목이 없습니다.<br>'
            f'이 규칙으로는 오늘 <b>아무것도 하지 않습니다</b>.</div>'
            f'<div style="font-size:27px;color:{C["sub"]};font-weight:700;'
            f'margin-top:24px">열 번 중 서너 번은 이런 날입니다 · 실제로 사는 날은 한 달에 한 번쯤</div></div>')
    ...
    for n, x in enumerate(후보[:4]):
        ...
        #    「안 산다」를 아래에 두면 자리가 달라 눈이 두 군데를 본다
        #      확정 전   매수 적정 범위 (파랑)
        #      확정·산다   ⬛ 지정가 (빨강)
        #      확정·안산다  안 산다 (흐림)
...
def build(out, days=DEFAULT_DAYS):
    ...
    home = (
        ...
        f'<div class="eyebrow">MORNING BRIEFING</div>'
        f'<h1>{BRAND}</h1>'
        f'<div class="sub">3분 만에 읽는 어제와 오늘의 시장</div>'
        ...
        f'<div class="note">투자 참고용이며 매수 권유가 아닙니다.<br>'
        f'수록 {len(kept)}일 ({oldest_kept} ~ {latest}) · '
        f'화면 생성 {built}</div>'
        f'</section>')
