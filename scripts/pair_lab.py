#!/usr/bin/env python3
r"""
pair_lab.py — **146차 · 신호마다 파는 시점을 따로** (2026-09-08 신설)

## 사용자 지적
```
「신호마다 사는 시점과 파는 시점이 다를 것 같아. 당연히도」
```
145차가 그걸 숫자로 보여줬다 (바탕: 1일 0.22% · 20일 4.08% · 90일 9.60%):
```
                   1일    5일   20일   40일   90일
외국인 5일 순매수    0.16  1.04  1.31  0.54  **-1.12**   ← 짧게 작동. 길면 사라진다
갭(다음날 시가)     0.11  1.11 -1.08 -1.43  **-4.51**   ← 5일까지만
소형주 우위        1.33  0.71 -7.48 -11.40 -22.91      ← 3일까지만
기관 5일 순매수    -0.07  0.40  2.06  4.02  **+4.91**   ← 길게 작동. 계속 커진다
회전율             0.12  0.68  3.61  4.78  **+6.35**   ← 길게
20일 낙폭          0.14 -2.01 -11.95 -16.77 -24.85     ← 길게 (우리가 쓰는 것)
```
⇒ 지금 우리는 **무슨 신호로 샀든 같은 매도**(반 +15%/40일 · 반 +40%/90일)를 쓴다.
  145차대로면 그건 **짧게 작동하는 신호를 죽이고 있다**

## 재는 것
```
신호   짧은 것 (외국인 5일 · 갭 · 소형주 우위)
      긴 것   (기관 5일 · 회전율 · 20일 낙폭)
매도   +5%/5일 · +8%/10일 · +10%/20일 · +15%/40일 · +25%/60일 · +40%/90일
      그리고 나눠팔기 조합 몇 가지
짝    신호 x 매도 를 전부 돌려 **어떤 짝이 맞는지** 본다
```
⚠️ 우리 후보(볼린저 -1.0 · 낙폭 -10)는 그대로 두고, **고르는 잣대**를 신호로 바꾼다.
   131차에서 고르기는 하루 1.5종목이라 잘 발동을 안 한다고 했지만,
   여기서는 **매도와 짝지어** 보는 것이라 뜻이 다르다

쓰는 법:
    python scripts\pair_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    print("=" * 108)
    print("  146차 · 신호마다 **파는 시점**을 따로")
    print(f"  후보 {len(사건):,}건 · 사는 조건은 그대로 (볼린저 -1.0 · 낙폭 -10 · 갭 -3.5)")
    print("=" * 108)

    표머리 = ("  " + " " * 30
              + "".join(f"{라[:11]:>13}" for 라, _ in 매도들[:6]))
    for 신라, 신fn in 신호들:
        if 신fn == "머리":
            print(f"\n  {신라}")
            continue
        print(f"\n  ── {신라} ──")
        print(f"  {'매도 방식':<32}{'끝 자산':>15}{'연평균':>9}"
              f"{'낙폭':>8}{'산 것':>7}{'돈÷낙폭':>9}")
        for 매라, 몫들 in 매도들:
            c = dict(기본틀)
            c["나눔"] = 몫들
            ...
            r = 시뮬(c)
            점 = r["연"] / max(abs(r["낙"]), 3.0)
            print(f"  {매라:<32}{r['끝']:>15,.0f}{r['연']:>8.2f}%"
                  f"{r['낙']:>7.1f}%{r['산']:>7}{점:>9.2f}")

    print("\n" + "=" * 108)
    print("  읽는 법")
    print("    - 신호마다 **가장 좋은 매도가 다르면** 145차 가설이 맞는 것이다")
    print("    - 전부 같은 매도가 1등이면 **신호와 매도는 상관없다**는 뜻이다")
    print("    - ⚠️ 여기서 이겨도 4관문을 거쳐야 규칙이 된다")
    print("=" * 108)
    return 0
