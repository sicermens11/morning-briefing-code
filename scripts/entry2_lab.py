#!/usr/bin/env python3
r"""
entry2_lab.py — **161차 · 시장 지수와 매수 시각** (2026-09-08 신설)

## 왜 — 아침에 센 「좁게 잡은 여덟 개」 중 둘이다
```
시장 지수 = 코스닥만   147차의 「시장이 -10% 빠진 날엔 문턱을 푼다」에서
                     **코스닥만** 봤다. 코스피·업종지수는 안 재봤다
매수 시각 = 09:00 시가만   종가·다음날은 한 번도 안 재봤다
```
사용자: 「범위 좁힌 것도 테스트에 반영하고!」

## ⚠️ 먼저 밝혀둘 것 — **분봉이 없다**
```
우리에게 있는 값:  시가 · 고가 · 저가 · 종가 (하루 4개뿐)
그래서 09:05·09:30·11:00 은 **잴 방법이 없다**
잴 수 있는 것: 시가 · 종가 · 다음날 시가 · 다음날 종가 · 사흘 뒤
```
「매수 시각을 넓게 쟀다」고 말하면 안 된다. **하루 안의 시각은 못 잰다**
...
## 재는 것
```
A 국면 지수    코스닥 · 코스피 · 그 종목 시장 · 업종지수 · 소형우위
B 국면 문턱    -5 ~ -20%  x  느슨하게 풀 낙폭 -3 ~ -8%
C 매수 시각    그날 시가(지금) · 그날 종가 · 다음날 시가 · 다음날 종가 · 사흘 뒤 종가
D 시각 x 국면  둘을 같이
```

쓰는 법:
    python scripts\entry2_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    # ── 매수 시각별 「산 값」을 미리 붙인다 ──
    #    ⚠️ 분봉이 없어 **하루 안의 시각은 못 잰다.** 시가·종가·다음날뿐이다
    print("  매수 시각별 값 붙이는 중...", flush=True)

    def _시가(i2, code):
        v = 주가[날[i2]].get(code)
        b = (비.get(날[i2]) or {}).get(code)
        return (v[0] * b[0]) if (v and b) else None

    def _종가(i2, code):
        v = 주가[날[i2]].get(code)
        return v[0] if v else None

    시각들 = (("그날 시가 (지금)", lambda i2, c: _시가(i2, c), 0),
              ("그날 종가", lambda i2, c: _종가(i2, c), 0),
              ("다음날 시가", lambda i2, c: _시가(i2 + 1, c), 1),
              ("다음날 종가", lambda i2, c: _종가(i2 + 1, c), 1),
              ("사흘 뒤 종가", lambda i2, c: _종가(i2 + 3, c), 3))
    for x in 사건:
        i2 = x["인"]
        for 라, fn, 미룸 in 시각들:
            try:
                x[f"산값_{라}"] = fn(i2, x["code"]) if i2 + 미룸 < len(날) else None
            except (IndexError, TypeError):
                x[f"산값_{라}"] = None
            x[f"산날_{라}"] = i2 + 미룸

    def 앞수익(x, n, 시각="그날 시가 (지금)"):
        """산 값에서 n거래일 뒤까지. 사라졌으면 **-50% 로 센다**"""
        a = x.get(f"산값_{시각}")
        if not a or a <= 0:
            return None
        j = x[f"산날_{시각}"] + n
    ...
    def 바탕(x, 낙문=-10.0):
        return (크기통과(x) and 대금통과(x) and 볼통과(x) and 재무통과(x)
                and x["낙폭20"] <= 낙문)
    ...
    def 세기(라, fn, 최소=150, 시각=None):
        ...
        for 기 in 기간들:
            if 시각:
                v = [z for z in (앞수익(x, 기, 시각) for x in 칸)
                     if z is not None]
            else:
    ...
    print("\n" + "=" * 120)
    print("  161차 · **국면 지수**와 **매수 시각** — 근거 없이 좁혀놨던 둘")
    print(f"  후보 {len(사건):,}건 · 바탕(아무 종목·아무 날) 20일 44.7% · +0.65%")
    print("=" * 120)

    붙 = {k: sum(1 for x in 사건 if x.get(k) is not None)
          for k in ("시장낙폭", "섹터낙폭", "소형우위")}
    print(f"\n  붙은 것 — 시장낙폭 {붙['시장낙폭']:,} · 섹터낙폭 "
          f"{붙['섹터낙폭']:,} · 소형우위 {붙['소형우위']:,} / {len(사건):,}")

    # ── A 국면 지수를 바꾸면 ──
    print("\n  ── A **어느 지수로 국면을 볼까** (그 지수가 -10% 넘게 빠진 날) ──")
    print("     ⚠️ 147차는 코스닥만 봤다. 그 규칙: 시장 -10%↓ 이면 낙폭 문턱을 -5%로 푼다")
    print(머2)
    지수들 = (("그 종목의 시장 (지금)", "시장낙폭"),
              ("업종지수", "섹터낙폭"),
              ("소형우위 (소형-대형)", "소형우위"))
    세기("국면 안 봄 · 낙폭 -10% (지금 규칙)", lambda x: 바탕(x))
    for 라, 키 in 지수들:
        세기(f"{라} -10%↓ 인 날만",
             lambda x, k=키: 바탕(x) and x.get(k) is not None and x[k] <= -10)
        세기(f"{라} -10%↓ 이면 낙폭 -5% 로 품",
             lambda x, k=키: (크기통과(x) and 대금통과(x) and 볼통과(x)
                             and 재무통과(x) and x.get(k) is not None
                             and (x["낙폭20"] <= (-5.0 if x[k] <= -10 else -10.0))))

    # ── B 국면 문턱 x 푸는 낙폭 ──
    print("\n  ── B **국면 문턱**과 **풀어줄 낙폭** (그 종목의 시장 기준) ──")
    print(머2)
    for 국 in (-5, -8, -10, -15, -20):
        for 풀 in (-3, -5, -8):
            세기(f"시장 {국}%↓ 이면 낙폭 {풀}% 로 품",
                 lambda x, a=국, b=풀: (크기통과(x) and 대금통과(x)
                                       and 볼통과(x) and 재무통과(x)
                                       and x.get("시장낙폭") is not None
                                       and (x["낙폭20"]
                                            <= (b if x["시장낙폭"] <= a
                                                else -10.0))))
        print()
    ...
    # ── C 매수 시각 ──
    print("\n  ── C **언제 사나** (지금 규칙 그대로 · 산 값만 바꾼다) ──")
    print("     ⚠️ 분봉이 없어 **하루 안의 시각(09:05·09:30)은 못 잰다**")
    print(머2)
    for 라, _fn, _m in 시각들:
        세기(라, lambda x: 바탕(x), 시각=라)

    # ── D 시각 x 국면 ──
    print("\n  ── D 매수 시각 x 국면 (시장 -10%↓ 이면 낙폭 -5%) ──")
    print(머2)

    def 국면규칙(x):
        return (크기통과(x) and 대금통과(x) and 볼통과(x) and 재무통과(x)
                and x.get("시장낙폭") is not None
                and (x["낙폭20"] <= (-5.0 if x["시장낙폭"] <= -10 else -10.0)))

    for 라, _fn, _m in 시각들:
        세기(f"{라} · 147차 규칙", 국면규칙, 시각=라)

    print("\n" + "=" * 120)
    print("  읽는 법")
    print("    - A 에서 업종지수가 시장지수보다 나으면 **147차를 다시 봐야 한다**")
    print("    - C 에서 종가 매수가 시가보다 나으면 **아침에 안 사도 된다**는 뜻이다")
    print("      (그러면 08:50 동시호가 절차가 통째로 바뀐다 — 4관문 필수)")
    print("    - 무엇이든 바꾸려면 4관문을 먼저 지나야 한다")
    print("=" * 120)
    return 0
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      os.environ.get("LAB_OUT") or "2026-09-08_161차_국면지수와매수시각.txt")
