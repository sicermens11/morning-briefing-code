# ==============================================================
#  queue_pairs6.ps1 — **ECOS 재료 둘을 gate7 에 붙인 뒤 조합 재판정** (2026-09-16 13:55)
...
#  SIZE_HI 없음 (지금 기준선 그대로). 앞줄(queue_pairs5) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_pairs6_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs5_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs5 끝')
}

적기 "[0] 앞줄(queue_pairs5)이 끝나길 기다린다"
$분 = 0
...
적기 "[PAIRS6] 13쌍을 Ⓗ 위에 얹어 4관문 + 기존 OR 공시 - 시작"
