...
#  전체 기간. 앞줄(queue_pairs2) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_combo3_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs2_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs2 끝')
}

적기 "[0] 앞줄(queue_pairs2)이 끝나길 기다린다"
$분 = 0
...
적기 "[COMBO3_ECOS] 단독 이긴 재료 OR · 앞 0건 재료 종목 분할 - 시작"
$env:LAB_OUT = "2026-09-16_COMBO3_ECOS수출입.txt"
try { & $py "scripts\combo4_lab.py" 2>&1 | Select-Object -Last 4 | ForEach-Object { 적기 "    $_" } }
catch { 적기 "⚠️ [COMBO3] 터졌다: $($_.Exception.Message)" }
Remove-Item env:LAB_OUT -ErrorAction SilentlyContinue
$밖 = Join-Path "data\_labs" "2026-09-16_COMBO3_ECOS수출입.txt"
if (Test-Path $밖) { 적기 "[COMBO3] 끝 — $('{0:N0}' -f (Get-Item $밖).Length) B" } else { 적기 "⚠️ [COMBO3] 결과 파일이 없다" }
적기 "===== queue_combo3 끝 ====="
