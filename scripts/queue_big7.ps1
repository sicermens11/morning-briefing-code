...
#  전체 기간. 앞줄(queue_pairs9) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_big7_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
적기 "[BIG7_대형주2000] 단독 이긴 재료 OR · 앞 0건 재료 종목 분할 - 시작"
$env:BIG_LO = "2000"
$env:LAB_OUT = "2026-09-17_BIG7_대형주2000.txt"
try { & $py "scripts\combo4_lab.py" 2>&1 | Select-Object -Last 4 | ForEach-Object { 적기 "    $_" } }
catch { 적기 "⚠️ [BIG7_대형주2000] 터졌다: $($_.Exception.Message)" }
Remove-Item env:LAB_OUT -ErrorAction SilentlyContinue
Remove-Item env:BIG_LO -ErrorAction SilentlyContinue
$밖 = Join-Path "data\_labs" "2026-09-17_BIG7_대형주2000.txt"
if (Test-Path $밖) { 적기 "[BIG7_대형주2000] 끝 — $('{0:N0}' -f (Get-Item $밖).Length) B" } else { 적기 "⚠️ [BIG7_대형주2000] 결과 파일이 없다" }
적기 "===== queue_big7 끝 ====="
