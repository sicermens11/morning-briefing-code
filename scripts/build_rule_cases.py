...
def _지수낙폭표():
    """{날짜: {"KOSPI": (낙20, 낙60, 120일선대비), "KOSDAQ": (…)}}"""
    import glob as _g
    계 = {"KOSPI": [], "KOSDAQ": []}
    날들 = []
    ...
    for i, 날 in enumerate(날들):
        ...
        for k, xs in 계.items():
            ...
            n120 = None
            if i >= _지수120평창:
                창 = [z for z in xs[i - _지수120평창:i] if z > 0]
                if 창:
                    n120 = (xs[i] / (sum(창) / len(창)) - 1) * 100
            칸[k] = (n20, n60, n120)
        표[날] = 칸
    return 표
...
def main():
    # ⭐ 30 -> 999 (2026-09-14 저녁). 화면은 [:3] 만 쓰지만 월별·요일별 분해에 **전부**가 필요하다.
    #    161건이면 60KB — 그냥 다 남긴다 (그때마다 7분짜리 재생성을 또 돌리지 않게)
    개수 = 999
    if "--개수" in sys.argv:
