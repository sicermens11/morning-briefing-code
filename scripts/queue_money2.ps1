...
#  전체 기간. 앞줄(queue_money1) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_money2_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_money1_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_money1 끝')
}

적기 "[0] 앞줄(queue_money1)이 끝나길 기다린다"
$분 = 0
...
적기 "[MONEY_2000_10000] 단독 이긴 재료 OR · 앞 0건 재료 종목 분할 - 시작"
$env:BIG_LO = "2000"
$env:BIG_HI = "10000"
$env:LAB_OUT = "2026-09-18_MONEY_2000_10000.txt"
try { & $py "scripts\combo4_lab.py" 2>&1 | Select-Object -Last 4 | ForEach-Object { 적기 "    $_" } }
catch { 적기 "⚠️ [MONEY_2000_10000] 터졌다: $($_.Exception.Message)" }
Remove-Item env:LAB_OUT -ErrorAction SilentlyContinue
Remove-Item env:BIG_LO -ErrorAction SilentlyContinue
Remove-Item env:BIG_HI -ErrorAction SilentlyContinue
Remove-Item env:BIG_LO -ErrorAction SilentlyContinue
$밖 = Join-Path "data\_labs" "2026-09-18_MONEY_2000_10000.txt"
if (Test-Path $밖) { 적기 "[MONEY_2000_10000] 끝 — $('{0:N0}' -f (Get-Item $밖).Length) B" } else { 적기 "⚠️ [MONEY_2000_10000] 결과 파일이 없다" }
적기 "===== queue_money2 끝 ====="
