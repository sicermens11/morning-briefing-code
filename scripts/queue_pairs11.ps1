# ==============================================================
#  queue_pairs11.ps1 — **달력 매수·매도** 「X 계절에 사서 Y 계절 끝에」 16조합 (2026-09-17 15:30 · 사용자 「다양하게 한판」)
#
...
#  SIZE_HI 없음 (지금 기준선 그대로). 앞줄(queue_pairs10) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_pairs11_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs10_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs10 끝')
}

적기 "[0] 앞줄(queue_pairs10)이 끝나길 기다린다"
$분 = 0
...
적기 "[PAIRS11] 13쌍을 Ⓗ 위에 얹어 4관문 + 기존 OR 공시 - 시작"
$env:BASE_GAP = "표본만+실전표본"
$env:BASE_RELGAP = "-3.5"
$env:BASE_SELL = "0.4,15,40 / 0.6,40,90"
$env:BASE_PICKS = "120"
$env:LAB_OUT = "2026-09-17_PAIRS11_달력매도.txt"
...
$밖 = Join-Path "data\_labs" "2026-09-17_PAIRS11_달력매도.txt"
