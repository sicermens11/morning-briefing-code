...
def quant_view(q):
    ...
    아1 = (_상한말 + ((f'<div style="background:#2a1c1a;border-left:9px solid #ef7a6c;'
           ...
           # ⭐⭐ **목록 차례를 밝힌다** (2026-09-14 지시).
           #    2026-09-14 에 위 넷이 상대갭 +0.82 / +1.01 / +0.04 / +0.04 로
           #    **전부 양수**였다. 규칙은 「남들보다 3.5%p 더 빠진 것」이니
           #    읽는 사람은 음수를 기대한다 — **왜 얕은 게 위에 있나**를
           #    화면이 스스로 말해야 한다.
           #    까닭: 목록은 **전날 08:00 후보 차례**이고, 상대갭은 그 뒤
           #          **09:00 동시호가**에 붙는다. 둘은 순서가 다르다
           #    ⚠️ **`확정` 으로 가르면 안 된다** — 2026-09-14 는 동시호가를 이미
           #       쟀으니 `확정` 이 참인데도 산 게 없어 **목록은 후보**였다.
           #       가르는 것은 `살것` 이다(`보일것 = 살것 if 살것 else 후보[:4]`).
           #       산 종목을 보여 주는 날에는 이 말이 안 맞는다
           #    ⚠️ **한 줄을 넘기지 마라** — 2026-09-14 에 세 줄짜리로 넣었더니
