...
def _이미쟀나(끝):
    ...
    """
    _동 = (끝 or {}).get("동시호가") or {}
    # ⚠️⚠️ **잰시각만 보면 안 된다** (2026-09-14 저녁). 예약 AutoAuctionPrice(08:50 ·
    #    auto_0850.py)도 잰시각을 쓴다 — 그러면 08:55 회차가 「이미 판정했다」로
    #    건너뛰어 **판정이 아예 안 된다.** 판정의 표식은 record_pick 만 쓰는 `예상시장갭`
    if _동.get("예상시장갭") is None:
        return False
    잰 = str(_동.get("잰시각") or "")
...
def main():
    ...
    for x in 후보:
        ...
        # ⚠️⚠️ **하한가/상한가로 찍힌 값은 버린다** (2026-09-15 아침에 당했다).
        #    호가창이 비면 예상체결가가 **정확히 하한가**로 찍힌다.
        #      나우IB       전날 1,085원 → 예상   760원(-29.95%)  실제 시가 +0.55%
        #      에이티넘인베스트  전날 2,125원 → 예상 1,490원(-29.88%)  실제 시가 -0.47%
        #    둘 다 `전날 x 0.7` **정확히** 그 값이었고, 08:50→08:55 에 **안 움직였다**.
        #    ⚠️ 우리 규칙은 「**많이 빠진 것**을 산다」라서 이 가짜가
        #       **정확히 우리가 찾는 모양**으로 들어온다 — 그날 둘 다 「살 것」에 들었다.
        #    ⚠️ 설령 **진짜** 하한가라 해도 사면 안 된다 —
        #       하한가로 시작할 종목에 -3.5% 지정가를 걸면 바로 체결되고 -30% 를 맞는다.
        #       가짜든 진짜든 **버리는 쪽이 옳다**
        _어제 = x.get("어제종가") or 0
        if _어제 and abs(가 - _어제 * 0.7) / _어제 < 0.005:
            빈 += 1
            찍기(f"    ⚠️ {x['이름'][:12]}({code}) 예상 {가:,.0f}원이 "
                 f"**하한가 그대로**다 ({갭:+.2f}%) — 호가가 비었다는 뜻이라 버린다")
            continue
        if _어제 and abs(가 - _어제 * 1.3) / _어제 < 0.005:
            빈 += 1
            찍기(f"    ⚠️ {x['이름'][:12]}({code}) 예상 {가:,.0f}원이 "
                 f"**상한가 그대로**다 ({갭:+.2f}%) — 버린다")
            continue
        # ⭐ 예상 갭 하한 (2026-09-16) — 하한가 「정확히」가 아니어도 그 근처(−25%↓)면 버린다.
        #    09-16 아이패밀리에스씨 −27.74% 가짜가 「살 것」에 들었다. 진짜 폭락이라도 안 산다 (rule_def 주석)
        if 갭 is not None and 갭 <= R.예상갭하한:
            빈 += 1
            찍기(f"    ⚠️ {x['이름'][:12]}({code}) 예상 {가:,.0f}원 ({갭:+.2f}%) — "
                 f"**{R.예상갭하한:g}% 아래**라 버린다 (얇은 호가거나 진짜 폭락 · 둘 다 안 산다)")
            continue
        # ⭐ 예상체결**량**을 같이 남긴다 (2026-09-15). 여태 받아 놓고 버렸다 —
        #    량이 아주 적으면 그 예상가는 못 믿는다. 문턱은 며칠 쌓아 보고 정한다
        try:
            _량 = float(량 or 0)
        except (TypeError, ValueError):
            _량 = 0.0
        찍기(f"    · {x['이름'][:12]}({code}) 전날 {x['어제종가']:,}원 → "
             f"예상 {가:,.0f}원 ({갭:+.2f}%) · 예상량 {_량:,.0f}"
             if 갭 is not None else
             f"    · {x['이름'][:12]}({code}) 예상 {가:,.0f}원 · 예상량 {_량:,.0f}")
    ...
    if r.returncode == 0:
        ...
        for L in 올[-2:]:
            찍기(f"    {L.strip()[:150]}")
        if p2.returncode != 0:
            찍기(f"  ⚠️ 재게시 실패 (코드 {p2.returncode}) — **판정은 됐는데 화면엔 안 올라갔다**")
            # ⚠️⚠️ 2026-09-16: 재게시가 실패했는데 끝 코드 0 을 돌려줘 예약 작업 결과가 「성공」이었다.
            #    09-16 08:56 「오늘 살 것 3개」가 화면에 안 올라갔는데 아무도 몰랐다. 실패는 실패로 돌려준다
            찍기("===== 끝 · 코드 2 (재게시 실패) =====")
            return 2
