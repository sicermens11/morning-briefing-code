#!/usr/bin/env python3
r"""
gate6_lab.py — **190차 · 4관문을 「이길 확률」 잣대로** (2026-09-08 신설)
...
def main():
    ...
    # ⭐⭐ **190차 도전자** — 188차에서 나온 「같이 빠진 것」 (2026-09-09)
    #    「반도체는 관련 종목이 **같이 오르고 같이 내린다**」 (사용자)
    #    188차 실측: 대형주 멀쩡한데 나만 -20%↓ = 51.7%
    #               대형주도 빠졌고 나도 -20%↓ = **78.6%**  (26.9%p 차이)
    도전자 = [
        ("지금 규칙", 지금),
        ("① 업종지수 -10%↓ 인 날만",
         lambda x: 지금(x) and 있(x, "섹터낙폭") and x["섹터낙폭"] <= -10),
        ("② 업종지수 -15%↓ 인 날만",
         lambda x: 지금(x) and 있(x, "섹터낙폭") and x["섹터낙폭"] <= -15),
        ("③ **업종보다 덜 빠진 것**",
         lambda x: 지금(x) and 있(x, "섹터대비") and x["섹터대비"] >= 3),
        ("④ 업종과 비슷하게 빠진 것",
         lambda x: 지금(x) and 있(x, "섹터대비") and x["섹터대비"] >= -3),
        ("⑤ **혼자 튄 것을 뺀다** (업종보다 -10%p↓ 제외)",
         lambda x: 지금(x) and 있(x, "섹터대비") and x["섹터대비"] >= -10),
        ("⑥ 시장 -5%↓ 인 날만",
         lambda x: 지금(x) and 있(x, "시장낙폭") and x["시장낙폭"] <= -5),
        ("⑦ 업종 -10%↓ **또는** 시장 -5%↓",
         lambda x: 지금(x) and ((있(x, "섹터낙폭") and x["섹터낙폭"] <= -10)
                               or (있(x, "시장낙폭") and x["시장낙폭"] <= -5))),
    ]
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      os.environ.get("LAB_OUT") or "2026-09-09_190차_4관문_같이빠진것.txt")
