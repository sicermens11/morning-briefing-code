...
def 회차별():
    ...
    for 회 in sorted(회차별오차):
        ...
        # ⭐ 2026-09-16: σ 4.5%p 는 가짜 예상체결가(−30%) 두어 건이 키운 값이다. 예상갭하한 −25 로 그런 건 이제 안 산다.
        #    |오차| > 10 을 걸러낸 σ 를 같이 찍는다 — 문턱 −3.5 → −3.0 검토는 이 값으로
        w = [z for z in v if abs(z) <= 10]
        if len(w) >= 2 and len(w) < len(v):
            표w = st.pstdev(w)
            판w = ("σ ≤ 0.5 — 문턱 -3.0 검토" if 표w <= 0.5 else "σ ≥ 1.0 — 못 믿는다" if 표w >= 1.0 else "0.5~1.0 · 더 모은다")
            print(f"    {'':<8}{len(w):>6}{st.mean(w):>+9.2f}{표w:>9.2f}{st.median([abs(z) for z in w]):>11.2f}{max(w, key=abs):>+9.2f}   ↳ |오차|>10 인 {len(v) - len(w)}건 걸러내면: {판w}")
