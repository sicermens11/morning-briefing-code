...
def main():
    ...
    def _때로들어옴(x):
        ...
        ⚠️ `_시7`·`_시낙` 은 한참 아래에서 정의된다. 파이썬은 **부를 때**
           이름을 찾으므로 괜찮다 — 첫 시뮬 호출이 그보다 뒤다
        """
        return _시7(x) or _시낙(x, 60, -10)
    ...
    _바탕c = {"상대갭": _밑상대갭, "목표": 20,
              "최대보유": R.앞몫기한, "비중": 0.20,
              "하루상한": R.하루최대종목,
              "시총하한": R.시총하한억, "시총상한": R.시총상한억,
              "대금하한": R.대금하한억, "거래량하한": 0, "회전율하한": 0.0,
              # 현행: 반 +15%(40일) · 반 +40%(90일) — rule_def 하나에서
              "나눔": R.몫들}
        ...
        if v is None:
            v = (0 if not 문통과(x) else
                 ((1 if (x["볼린저"] <= R.볼린저문턱
                  ...
                  + (1 if 섹터맞나(x) else 0)
                  + (1 if (_시7(x) or _시낙(x, 60, -10)) else 0)))
            x["_겹친"] = v
        return v
    ...
    def _H60(x):
        ...
        return _볼60지금(x) or _시7(x) or _시낙(x, 60, -10)
    ...
    _기237 = 시뮬(_c(_H))
    표(_기237, "[견줌] Ⓗ (볼20 · 지금)")
    표(시뮬(_c(_H60)), "Ⓗ 인데 **볼60**", _기237)
    표(시뮬(_c(lambda x: 지금(x) or _시7(x) or _시낙(x, 60, -10))),
      "  (같은 것 다시 · 검산)", _기237)
        ...
        _도전 = (
            ...
            ("⭐ Ⓗ 인데 크기 상한만 해제",
             lambda x: (재무통과(x) and 대금통과(x) and x["시총억"] >= 500
                        and ((x["볼린저"] <= R.볼린저문턱
                              and x["낙폭20"] <= R.낙폭20문턱)
                             or 섹터맞나(x) or _시7(x) or _시낙(x, 60, -10)))),
    ...
    for _라9, _잣9 in _볼것253[:2]:
        _이9, _짐9, _무9 = 0, 0, 0
        print(f"\n   [{_라9}]  {'해':<7}{'㉠ 후보만':>15}{'도전':>15}"
              f"{'차이':>9}{'㉠ 낙':>8}{'도전 낙':>9}")
        for _y9 in _해253:
            try:
                _기y = 시뮬(_c(_H, 제약없음=True), 시작년=_y9, 끝년=_y9)
    ...
    for _e264 in (0.0, 0.3, 0.5, 1.0, 2.0):
        ...
        for _s264 in range(3 if _e264 else 1):
            _r = 시뮬(_c(_H, 제약없음=True), 오차=_e264, 시드=_s264)
            if _r:
                _벌.append(_r)
        if not _벌:
            continue
    ...
    print("     ⚠️ 실전은 화면에 40개를 싣는다. 늘리려면 화면도 같이 바뀐다")
    print("     ⚠️ 판정은 **끝 자산과 낙폭**. 기회가 늘어도 돈이 줄면 안 된다")
    print("=" * 108)

    print("\n  ── A ⭐⭐⭐ **자르는 개수** (제약 있는 판 / 없는 판) ──")
    print(머239)
    _기261 = _둘(f"{_후보수}개 (지금)", _c(_H))
    ...
    _넘는날 = sum(1 for i in 묶 if len(_그날후보(i)) > _후보수)
    _산날 = sum(1 for i in 묶 if _그날후보(i))
    print("\n     후보가 %d개를 넘는 날 **%s일** / 후보가 있던 %s일"
          % (_후보수, format(_넘는날, ","), format(_산날, ",")))
