...
def main():
    ...
    if not _쓸수있나:
        ...
        print(머239)
        _둘(f"{_밑글자} (지금 · 견줌)", _c(_H))
        for _라7, _f7 in (("**OR** 중형", _중형규),
                          ("**OR** 대형", _대형규),
                          ("**OR** 초대형", _초대규),
                          ("**OR** 큰 것 셋 다", _큰셋)):
            _둘(f"{_밑글자} {_라7}",
                _c(lambda x, g=_f7: _H(x) or g(x), 시총하한=0, 시총상한=999999))

        print("\n     ⚠️ **산 것이 늘었나**를 먼저 본다. 안 늘었으면 돈이 늘어도 기각이다")
        print("        (오늘 ③에서 죽은 넷 중 셋이 「기회를 깎아 이긴 것」이었다)")

        print("\n  ── C ⭐⭐ **걷기 검증** (앞 2010~2020 / 뒤 2021~2026) ──")
        for _시7, _끝7, _라8 in (("2010", "2021", "[앞 2010~2020]"),
                                 ("2021", "2027", "[**뒤 2021~2026**]")):
            print(f"\n   {_라8}")
            print(머)
            _기8 = 시뮬(_c(_H, 제약없음=True), 시작년=_시7, 끝년=_끝7)
            표(_기8, f"[견줌] {_밑글자}")
            for _라7, _f7 in (("OR 중형", _중형규), ("OR 대형", _대형규),
                              ("OR 초대형", _초대규), ("OR 큰 것 셋 다", _큰셋)):
                표(시뮬(_c(lambda x, g=_f7: _H(x) or g(x), 제약없음=True,
                          시총하한=0, 시총상한=999999),
                        시작년=_시7, 끝년=_끝7), f"{_밑글자} {_라7}", _기8)
        ...
        for _y8 in range(2016, 2027):
            _a8 = 시뮬(_c(_H, 제약없음=True),
                       시작년=str(_y8), 끝년=str(_y8 + 1))
            _b8 = 시뮬(_c(lambda x: _H(x) or _큰셋(x), 제약없음=True,
                          시총하한=0, 시총상한=999999),
                       시작년=str(_y8), 끝년=str(_y8 + 1))
