...
#  전체 기간. 앞줄(queue_combo6) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_combo7_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_combo6_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_combo6 끝')
}

적기 "[0] 앞줄(queue_combo6)이 끝나길 기다린다"
$분 = 0
...
적기 "[COMBO7_계절] 단독 이긴 재료 OR · 앞 0건 재료 종목 분할 - 시작"
$env:LAB_OUT = "2026-09-16_COMBO7_계절.txt"
...
$밖 = Join-Path "data\_labs" "2026-09-16_COMBO7_계절.txt"
