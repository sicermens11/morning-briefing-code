...
def main():
    ...
    # ── ⭐⭐⭐ **규모 띠별 재료** (2026-09-17 · 사용자 「2000억 이상을 세분화하던가 해서」) ──
    #    띠마다 바탕이 다르다 — 그 띠 안에서만 견준다. 지금까지는 전 종목을 섞어 재서
    #    수로 압도하는 300~2,000억 의 몫만 보였다
    print("\n" + "=" * 122)
    ...
    def _재기B(자리들):
        ...
        n뒤, w뒤 = n - n앞, w - w앞
        return (n, w / n * 100,
                (w앞 / n앞 * 100) if n앞 >= 300 else None,
                (w뒤 / n뒤 * 100) if n뒤 >= 300 else None)
    ...
    for _lo, _hi, _라B in _띠들:
        ...
        _n, _w, _, _ = _재기B(_자리)
        _바탕B[_라B] = _w
        _종수 = len({사건[i2]["code"] for i2 in _자리}) if _자리 else 0
        print(f"     {_라B:<22}{_종수:>7,}{_n:>11,}{_n / 해수:>8,.0f}"
              + (f"{_w:>9.1f}%" if _w is not None else f"{'·':>10}"))

    for _lo, _hi, _라B in _띠들:
        _자리 = set(_띠자리[_라B])
        if len(_자리) < 50000:
            print(f"\n   [{_라B}] 사건 {len(_자리):,}건 — **표본 부족**이라 건너뛴다")
            continue
        _바 = _바탕B[_라B]
        _벌 = []
        for _라1, _마 in 조건.items():
            _칸 = [i2 for i2 in _자리뽑기(_마) if i2 in _자리]
            if len(_칸) < 500:
                continue
            _n, _w, _앞, _뒤 = _재기B(_칸)
            if _w is None:
                continue
            _차 = _w - _바
            _같 = (_앞 is not None and _뒤 is not None
                   and (_앞 - _바) * (_뒤 - _바) > 0 and abs(_앞 - _바) > 1 and abs(_뒤 - _바) > 1)
            _벌.append((abs(_차), _차, _라1, _n, _w, _앞, _뒤, _같))
        _벌.sort(reverse=True)
        print(f"\n   [{_라B}]  바탕 {_바:.1f}%  ·  재료 {len(_벌)}가지 잼  ·  차이 큰 순 8개")
        print(f"     {'재료':<24}{'건수':>9}{'이김':>8}{'바탕대비':>9}{'앞':>8}{'뒤':>8}   앞뒤같나")
        for _, _차, _라1, _n, _w, _앞, _뒤, _같 in _벌[:8]:
            print(f"     {_라1:<24}{_n:>9,}{_w:>7.1f}%{_차:>+9.1f}"
                  f"{(f'{_앞 - _바:+.1f}' if _앞 is not None else '·'):>8}"
                  f"{(f'{_뒤 - _바:+.1f}' if _뒤 is not None else '·'):>8}   {'✅' if _같 else '❌'}")
    print("     ⚠️ ✅ 라도 아직 「이길 확률」뿐이다 — 돈(③)으로 넘겨야 뜻이 산다")
    print("=" * 122)

    print("\n  ── D **지금 규칙**과 견주면 ──")
    # ⭐ 대형주 판이면 견줌도 대형주 범위로 (2026-09-17)
    _돈하한2 = _BIG_LO if _BIG_LO else R.시총하한억
    _돈상한2 = (_BIG_HI if _BIG_HI else 999999) if _BIG_LO else R.시총상한억
    if _BIG_LO:
        print(f"     ⚠️ **대형주 전용 판** — 사건·바탕·돈이 전부 시총 {_BIG_LO:,.0f}억 "
              f"{('~ %s억' % f'{_BIG_HI:,.0f}') if _BIG_HI else '이상'} 안에서만이다")
    print(머2)
    # ⭐ 재기() 는 마스크(int)든 자리 목록이든 둘 다 받는다 (2026-09-14 밤)
    지금 = [i2 for i2, x in enumerate(사건)
            if (x.get("재통과") == 1.0 and _돈하한2 <= x["시총억"] < _돈상한2
    ...
    for _i2, _x in enumerate(사건):
        _x["_i"] = _i2
    # ⭐ 대형주 판이면 돈 시뮬도 대형주를 산다 (2026-09-17)
    _돈하한 = _BIG_LO if _BIG_LO else R.시총하한억
    _돈상한 = (_BIG_HI if _BIG_HI else 999999) if _BIG_LO else R.시총상한억
    _밑E = {"시총하한": _돈하한, "시총상한": _돈상한,
    ...
    def _지금E(x):
        ...
        창값이 combo4 사건에 없다. 그래서 여기 견줌은 gate7 Ⓗ(2.541억)보다 조금 낮다.
        통과한 것은 어차피 gate7(전체 규칙)에서 다시 잰다"""
        if x.get("재통과") != 1.0:
            return False
        if x["볼린저"] <= R.볼린저문턱 and x["낙폭20"] <= R.낙폭20문턱:
            return True
        _m20, _m60 = x.get("시장낙폭"), x.get("시장낙60")
        return ((_m20 is not None and _m20 <= R.지수낙20문턱)
                or (_m60 is not None and _m60 <= R.지수낙60문턱))

    print(머)
    _기E = 시뮬(dict(_밑E, 거름=_지금E))
    표(_기E, "지금 (기존+시장 · 섹터 없음)")
    if not _산것:
    ...
    # ── E-2 ⭐⭐⭐ **문턱을 조이면 돈이 되나** (2026-09-18 밤) ──
    #    RISE2: 낙폭둔화↑ 가 승률 +5.1%p 인데 돈은 −99% 였다. 재료가 넓어(전체의 20%)
    #    하루 4자리를 매일 채워 좋은 것을 밀어낸 탓이다. 조이면 달라지나
    print("\n" + "=" * 122)
    print("  ── E-2 ⭐⭐⭐ **문턱을 조이면 돈이 되나** — 같은 재료를 위/아래 20% · 10% · 5% 로 ──")
    print("     승률이 좋은데 돈이 나쁜 재료는 대개 **너무 넓어서** 하루 4자리를 평범한 것으로 채운다")
    print("=" * 122)
    _바탕20 = None
    try:
        _n0, _낸0 = 재기(list(range(len(사건))))
        _바탕20 = _낸0.get(20)[0] if _낸0.get(20) else None
    except Exception:  # noqa: BLE001
        pass
    if _바탕20 is None:
        print("     ⚠️ 바탕을 못 냈다 — 건너뛴다")
    else:
        # 승률이 바탕+3%p 넘은 재료 (이름에서 ↑↓ 를 떼어 원재료로)
        _좋은재 = []
        for _라1, _마1 in 조건.items():
            _n1, _낸1 = 재기(_마1)
            if not _낸1.get(20):
                continue
            if _낸1[20][0] - _바탕20 >= 3.0:
                _좋은재.append((_낸1[20][0] - _바탕20, _라1[:-1], _라1[-1]))
        _좋은재.sort(reverse=True)
        print(f"     바탕 {_바탕20:.1f}% · **바탕+3%p 넘은 재료 {len(_좋은재)}개** — 위 8개를 세 문턱으로 잰다")
        _기T = 시뮬(dict(_밑E, 거름=_지금E))
        if _기T:
            print(f"\n     {'재료':<20}{'문턱':<8}{'건수':>10}{'끝 자산':>16}{'지금의%':>9}{'낙폭':>8}{'산 것':>7}  판정")
            print(f"     {'[견줌] 지금 규칙':<28}{'':>10}{_기T['끝']:>15,.0f}원{100:>8.0f}%{_기T['낙']:>7.1f}%{_기T['산']:>7}")
            for _차T, _재T, _방T in _좋은재[:8]:
                _vals = sorted(z for z in (_계산재료[_재T](x) if _재T in _계산재료 else x.get(_재T)
                                           for x in 사건) if z is not None)
                if len(_vals) < len(사건) * 0.3:
                    continue
                for _몫T, _라T in ((0.20, "위 20%"), (0.10, "위 10%"), (0.05, "위 5%")):
                    _컷 = _vals[int(len(_vals) * (1 - _몫T))] if _방T == "↑" else _vals[int(len(_vals) * _몫T)]
                    def _거름T(x, 재=_재T, 방=_방T, 컷=_컷):
                        z = _계산재료[재](x) if 재 in _계산재료 else x.get(재)
                        if z is None:
                            return False
                        return (z >= 컷) if 방 == "↑" else (z <= 컷)
                    _f = (lambda x, g=_거름T: _지금E(x) or g(x))
                    _n2 = sum(1 for x in 사건 if _거름T(x))
                    _r = 시뮬(dict(_밑E, 거름=_f))
                    if not _r:
                        continue
                    _ok = (_r["산"] > _기T["산"], _r["끝"] > _기T["끝"], _r["낙"] > -10.0)
                    print(f"     {(_재T + _방T):<20}{_라T:<8}{_n2:>10,}{_r['끝']:>15,.0f}원"
                          f"{_r['끝'] / _기T['끝'] * 100:>8.0f}%{_r['낙']:>7.1f}%{_r['산']:>7}  "
                          f"{'✅ 셋 다' if all(_ok) else '❌'}")
        print("     ⚠️ 조이면 산 것이 줄 수 있다 — **산 것↑ · 돈↑ · 낙폭 −10% 안** 셋 다여야 뜻이 있다")
    print("=" * 122)
    ...
    # ── G ⭐⭐⭐ **사건 창 넓이** — 며칠 전부터 며칠 뒤까지 영향이 있나 (2026-09-16 · 사용자 「5일도 좁다고 보는거야?」) ──
    import bisect as _bG
    print("\n" + "=" * 122)
    print("  ── G ⭐⭐⭐ **사건 창 넓이** — 사건일로부터 거리별 20일 이김% (전체 사건 / 문 통과한 것) ──")
    print("     5일은 짐작이다. 어느 거리에서 바탕과 갈라지는지가 **창 넓이**다. 표본 500 미만은 「·」")
    print("=" * 122)
    _통G = [i for i, x in enumerate(사건) if x.get("재통과") == 1.0]
    _arr20 = 수익[20]
    ...
    def _이김G(idx):
        ...
        for i2 in idx:
            ...
            n += 1
            w += (z > 0)
        return n, (w / n * 100 if n else None)

    _바탕n, _바탕w = _이김G(range(len(사건)))
    _통n, _통w = _이김G(_통G)
    print(f"     바탕 전체 {_바탕n:,}건 {_바탕w:.1f}% · 문 통과 {_통n:,}건 {_통w:.1f}%")
    _통집 = set(_통G)
    _칸G = (("전 16~20", -20, -16), ("전 11~15", -15, -11), ("전 6~10", -10, -6), ("전 1~5", -5, -1),
            ("당일·후 1~5", 0, 5), ("후 6~10", 6, 10), ("후 11~15", 11, 15), ("후 16~20", 16, 20))
    _사건들G = [("FOMC 회의", _fomc회의자리), ("미국 CPI", _cpi자리), ("미국 고용", _고용자리),
              ("금통위 변경", _금통자리), ("한국 선거", _선거자리), ("미국 선거", _미선거자리),
              ("수출 발표(월초)", _월첫자리)]
    _실적자리G = {c: sorted(자리_날[d] for d in ds if d in 자리_날) for c, ds in _실적일.items()}
    _사건들G.append(("실적 공시(종목별 · 실전 불가)", None))
    print(f"\n     {'사건':<22}" + "".join(f"{k:>11}" for k, _, _ in _칸G))
    for 이름G, 자리G in _사건들G:
        if 자리G is not None and not 자리G:
            continue
        칸값 = {k: [[], []] for k, _, _ in _칸G}
        for i2, x in enumerate(사건):
            i = x["인"] - 1
            자 = 자리G if 자리G is not None else _실적자리G.get(x["code"])
            if not 자:
                continue
            q = _bG.bisect_right(자, i)
            후 = (i - 자[q - 1]) if q >= 1 else 999
            전 = (자[q] - i) if q < len(자) else 999
            d = 후 if 후 <= 전 else -전       # 가까운 쪽 · 뒤는 +, 앞은 −
            for k, a, b in _칸G:
                if a <= d <= b:
                    칸값[k][0].append(i2)
                    if i2 in _통집:
                        칸값[k][1].append(i2)
                    break
        for 줄, 라 in ((0, "전체"), (1, "문 통과")):
            셀 = []
            for k, _, _ in _칸G:
                n, w = _이김G(칸값[k][줄])
                셀.append(f"{w:>9.1f}%" if (w is not None and n >= 500) else f"{'·':>10}")
            print(f"     {(이름G if 줄 == 0 else '   ' + 라):<22}" + " ".join(셀))
    print("     ⚠️ 읽는 법: 바탕(전체/문 통과)보다 3%p 넘게 벌어진 칸이 이어지는 구간이 「창」이다. 선거·월드컵은 사건이 열 몇 번뿐이라 참고만")

    # ── H ⭐⭐ **업종 × 계절** (2026-09-16 · 사용자 「여름엔 공조 겨울엔 난방」) ──
    print("\n" + "=" * 122)
    print("  ── H ⭐⭐ **업종 × 계절** — 그 업종이 그 계절에 더 오르나 (20일 이김% · 업종 전체와 견줌) ──")
    print("     ⚠️ 테마(공조·난방 …) 소속 이력은 없다 — 섹터맵 업종으로 본다. 앞·뒤 반으로 갈라 같은 방향이면 ✅")
    print("=" * 122)
    _계절 = {"12": "겨울", "01": "겨울", "02": "겨울", "03": "봄", "04": "봄", "05": "봄",
            "06": "여름", "07": "여름", "08": "여름", "09": "가을", "10": "가을", "11": "가을"}
    _반H = len(사건) // 2
    _셀H = {}
    _업H = {}
    for i2, x in enumerate(사건):
        섹 = x.get("섹터")
        z = _arr20[i2]
        if not 섹 or z is None:
            continue
        계 = _계절[날[x["인"] - 1][4:6]]
        앞뒤 = 0 if i2 < _반H else 1
        for 키, 표 in (((섹, 계), _셀H), (섹, _업H)):
            v = 표.setdefault(키, [0, 0, 0, 0])      # n, w, n앞, w앞
            v[0] += 1
            v[1] += (z > 0)
            if 앞뒤 == 0:
                v[2] += 1
                v[3] += (z > 0)
    행 = []
    for (섹, 계), (n, w, n앞, w앞) in _셀H.items():
        if n < 500:
            continue
        N, W, N앞, W앞 = _업H[섹]
        전체 = w / n * 100
        업종 = W / N * 100
        차 = 전체 - 업종
        앞차 = (w앞 / n앞 * 100 - W앞 / N앞 * 100) if (n앞 >= 200 and N앞) else None
        뒤n, 뒤w, 뒤N, 뒤W = n - n앞, w - w앞, N - N앞, W - W앞
        뒤차 = (뒤w / 뒤n * 100 - 뒤W / 뒤N * 100) if (뒤n >= 200 and 뒤N) else None
        같음 = (앞차 is not None and 뒤차 is not None and (앞차 > 1) == (차 > 0) and (뒤차 > 1) == (차 > 0)) if 차 > 0 else \
              (앞차 is not None and 뒤차 is not None and 앞차 < -1 and 뒤차 < -1)
        행.append((차, 섹, 계, n, 전체, 업종, 앞차, 뒤차, 같음))
    행.sort(key=lambda t: -abs(t[0]))
    print(f"\n     {'업종':<18}{'계절':<6}{'건수':>8}{'그 계절':>9}{'업종 전체':>10}{'차이':>8}{'앞':>8}{'뒤':>8}   앞뒤 같나")
    for 차, 섹, 계, n, 전체, 업종, 앞차, 뒤차, 같음 in 행[:30]:
        if abs(차) < 3:
            break
        print(f"     {섹[:17]:<18}{계:<6}{n:>8,}{전체:>8.1f}%{업종:>9.1f}%{차:>+8.1f}"
              f"{(f'{앞차:+.1f}' if 앞차 is not None else '·'):>8}{(f'{뒤차:+.1f}' if 뒤차 is not None else '·'):>8}   {'✅' if 같음 else '❌'}")
    print(f"     … 업종×계절 칸 {len(_셀H)}개 중 건수 500↑ {len(행)}개 · 차이 ±3%p 넘는 것만 위에 (앞뒤 같은 방향 ✅ 이어야 믿는다)")
    print("=" * 122)
    return 0
