...
#  전체 기간. 앞줄(queue_pairs8) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_combo6_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs8_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs8 끝')
}

적기 "[0] 앞줄(queue_pairs8)이 끝나길 기다린다"
$분 = 0
...
적기 "[COMBO6_ETF괴리] 단독 이긴 재료 OR · 앞 0건 재료 종목 분할 - 시작"
$env:LAB_OUT = "2026-09-16_COMBO6_ETF괴리율.txt"
...
$밖 = Join-Path "data\_labs" "2026-09-16_COMBO6_ETF괴리율.txt"
