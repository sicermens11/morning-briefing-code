...
def check(date):
    ...
    오류, 경고 = [], []
    _판정검사(d, date, 오류, 경고)
    # ⚠️⚠️ **관망일(후보 0개)에 `종목` 이 빈 객체라 「비었거나 없는 키」로 게시가 막혔다** (2026-09-16).
    #    09-16 아침: 섹터 브리핑 후보 0개 → 웹은 09-15 페이지 그대로. 게다가 **08:56 퀀트 판정(오늘 살 것 3개)
    #    재게시까지 같은 검사에 막혔다.** 후보가 없는 날은 `종목` 이 비어 있는 것이 **맞는 상태**다 —
    #    SKILL 은 억지 후보를 금지한다. 후보가 있는 날에만 `종목` 을 필수로 본다
    _오늘픽 = (_day(date).get("picks") or [])
    빈 = [k for k in 필수
          if not (k == "종목" and not _오늘픽)
          and (not str(d.get(k, "")).strip() or d.get(k) in ([], {}))]
    if 빈:
