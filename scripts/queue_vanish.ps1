# ==============================================================
#  queue_vanish.ps1 — **상장폐지 −50% 가정 확인** (2026-09-16 17:20 · 사용자 「지금 테스트 끝나면 바로 돌리자」)
#  pairs3(같은 env · VANISH_KIND 끔)과 나란히 견준다. 공개매수·합병은 −50% 가 아니라 실제 값으로
#
...
#  SIZE_HI 없음 (지금 기준선 그대로). 앞줄(queue_pairs6) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_vanish_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs6_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs6 끝')
}

적기 "[0] 앞줄(queue_pairs6)이 끝나길 기다린다"
$분 = 0
...
적기 "[VANISH] 13쌍을 Ⓗ 위에 얹어 4관문 + 기존 OR 공시 - 시작"
$env:BASE_GAP = "표본만+실전표본"
$env:BASE_RELGAP = "-3.5"
$env:BASE_SELL = "0.4,15,40 / 0.6,40,90"
$env:BASE_PICKS = "120"
$env:VANISH_KIND = "1"
$env:LAB_OUT = "2026-09-16_VANISH_상장폐지가정.txt"
try { & $py "scripts\gate7_lab.py" 2>&1 | Select-Object -Last 4 | ForEach-Object { 적기 "    $_" } }
catch { 적기 "⚠️ [PAIRS] 터졌다: $($_.Exception.Message)" }
foreach ($k in "BASE_GAP", "BASE_RELGAP", "BASE_SELL", "BASE_PICKS", "VANISH_KIND", "LAB_OUT") { Remove-Item "env:$k" -ErrorAction SilentlyContinue }
$밖 = Join-Path "data\_labs" "2026-09-16_VANISH_상장폐지가정.txt"
