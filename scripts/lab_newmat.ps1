...
function 판($이름, $설명, $더할인자) {
    # ⭐ 이미 끝난 판은 건너뛴다 (2026-09-14 밤 — 재료를 늘려 다시 돌릴 때 헛일 방지)
    $결과0 = "data\_labs\2026-09-14_$이름.txt"
    if ((Test-Path $결과0) -and ((Get-Item $결과0).Length -gt 50000)) {
        적기 "[$이름] 이미 있다 ($('{0:N0}' -f (Get-Item $결과0).Length) B) — 건너뛴다"
        return
    }
    적기 "[$이름] $설명 - 시작"
...
판 "U_새재료_최근2년" "+ 배당·소액주주까지 (최근 2년 · dart-snap 은 2024~2025 뿐)" "--최근2년"
적기 "모두 끝"
