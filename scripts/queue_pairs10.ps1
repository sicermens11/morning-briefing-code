# ==============================================================
#  queue_pairs10.ps1 — **실전 기준선(소형 세 갈래 + ㉣ 큰 회사 섹터 갈래)에서 후보 2·3호 4관문 + 빈 날** (2026-09-16 19:10)
#  pairs7 은 시뮬 시총상한 2,000 이 ㉣ 을 다시 걸러 Ⓗ 가 소형만이었다 → _바탕c 상한을 열고 다시
#
...
#  SIZE_HI 없음 (지금 기준선 그대로). 앞줄(queue_pairs9) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_pairs10_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_pairs9_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_pairs9 끝')
}

적기 "[0] 앞줄(queue_pairs9)이 끝나길 기다린다"
$분 = 0
...
적기 "[PAIRS10] 13쌍을 Ⓗ 위에 얹어 4관문 + 기존 OR 공시 - 시작"
$env:BASE_GAP = "표본만+실전표본"
$env:BASE_RELGAP = "-3.5"
$env:BASE_SELL = "0.4,15,40 / 0.6,40,90"
$env:BASE_PICKS = "120"
$env:LAB_OUT = "2026-09-16_PAIRS10_여름빼고.txt"
...
$밖 = Join-Path "data\_labs" "2026-09-16_PAIRS10_여름빼고.txt"
