#!/usr/bin/env python3
r"""
gate8_lab.py — **197차 · 상관 무리의 「같이 빠짐」을 4관문에** (2026-09-09 밤 신설)

## 196차에서 나온 것 — 여기서 판정한다
```
성격                        나만 빠짐    **같이 빠짐**    차이
...
무리에 든 사건의 **56%가 테마형** · 업종형은 16%뿐이다
```
⚠️⚠️ 196차는 **재무 조건(잉여금·부채·흑자)이 없는** 잣대였다.
   여기서는 재무까지 넣고 **4관문(A 앞뒤·B 해마다·C 무작위 대조)**에 건다

## ⚠️ 무리는 **파일에서 읽는다**
```
corr_lab 이 `data/_무리_60.json` 에 남긴 것을 쓴다 (상관 문턱 0.6).
  · 무리는 **그 해가 시작되기 전 120거래일**로 만들었다 — 미리보기가 아니다
  · 시장 요인을 뺀 잔차로 쟀다 — 안 빼면 전 종목이 한 덩어리가 된다
  · 문턱 0.5 에선 200종목이 한 덩어리였다. **0.6 이 답이다**(195차)
```

## ⚠️ **기회 수를 반드시 같이 본다**
```
사용자: 「매도 타이밍과 자산 보유 현황도 중요하지만
        **그것도 때문에 상승 기회를 놓쳐서는 안돼**」
190차가 「7개 전부 통과」로 찍혔는데 기회가 **6~37%** 로 줄어 있었다
```
"""
import glob
import io
...
# ══ ⭐ 상관 무리 (197차) ══
def _무리읽기():
    r"""corr_lab 이 남긴 무리를 읽는다 -> {(해, 코드): 대표}

    ⚠️ 없으면 빈 dict — 그러면 무리 도전자들이 다 「표본 부족」으로 나온다.
       그때는 `python scripts\corr_lab.py` 를 MUNTEOK=0.6 으로 먼저 돌려야 한다
    """
    p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                     "data", "_무리_60.json")
    if not os.path.exists(p):
        print("  ⚠️⚠️ data/_무리_60.json 이 없다 — 무리 도전자는 표본 부족이 된다",
              flush=True)
        return {}, {}
    d = json.load(io.open(p, encoding="utf-8-sig"))
    속 = {}
    크기 = {}
    for 해, 무 in (d.get("해마다") or {}).items():
        for 대, 들 in 무.items():
            크기[(해, 대)] = len(들)
            for c in 들:
                속[(해, c)] = 대
    print(f"  무리 읽음 · 해 {len(d.get('해마다') or {})}개 ·"
          f" 무리-종목 {len(속):,}개 (문턱 {d.get('문턱')})", flush=True)
    return 속, 크기


def _업종쏠림표(속, 산업):
    r"""(해, 대표) -> 그 무리에서 **가장 많은 업종의 비율**

    ⚠️ 업종 분류는 무리를 **만드는 데는 안 쓴다**(뭉개져 있으니까).
       만들어진 무리가 **한 업종에 몰렸나(업종형) / 흩어졌나(테마형)**를 재는 데만 쓴다
    """
    묶 = {}
    for (해, c), 대 in 속.items():
        묶.setdefault((해, 대), []).append(c)
    난것 = {}
    for k, 들 in 묶.items():
        셈 = {}
        for c in 들:
...
def main():
    ...
    # ── ⭐ 상관 무리 붙이기 (197차) ──
    _속, _크기 = _무리읽기()
    _쏠림 = _업종쏠림표(_속, _산업) if _속 else {}
    for x in 사건:
        해 = x["해"]          # ⚠️ `_날짜` 키는 없다 (2026-09-10)
        대 = _속.get((해, x["code"]))
        x["무리"] = 대
        x["무리쏠림"] = _쏠림.get((해, 대)) if 대 else None
    # 그날 그 무리의 20일 낙폭 **중앙값**
    _묶 = {}
    for x in 사건:
        if x.get("무리") and x.get("낙폭20") is not None:
            _묶.setdefault((x["인"], x["무리"]), []).append(x["낙폭20"])
    _중 = {}
    for k, v in _묶.items():
        v.sort()
        _중[k] = v[len(v) // 2]
    for x in 사건:
        m = _중.get((x["인"], x.get("무리"))) if x.get("무리") else None
        x["무리낙폭"] = m
        x["무리대비"] = (x["낙폭20"] - m) if (m is not None
                                          and x.get("낙폭20") is not None) else None
    _붙 = sum(1 for x in 사건 if x.get("무리낙폭") is not None)
    print(f"  무리 붙은 것 {_붙:,}/{len(사건):,}"
          f" ({_붙/max(1,len(사건))*100:.0f}%)", flush=True)
    ...
    # ⭐⭐ **197차 도전자** — 196차에서 나온 무리 성격 (2026-09-09)
    #    ⚠️ 196차는 재무 조건이 없는 잣대였다. 여기서는 재무까지 넣고 다시 잰다
    def 무리있(x):
        return x.get("무리대비") is not None

    def 테마형(x):
        return x.get("무리쏠림") is not None and x["무리쏠림"] < 0.4

    def 업종형(x):
        return x.get("무리쏠림") is not None and x["무리쏠림"] >= 0.7

    # ⭐⭐ 217차 헬퍼 — **무리가 통째로 빠졌나** (2026-09-10)
    #  ⚠️ 지금 규칙을 안 거친다(OR 이므로). 그래서 재무·대금·크기는 **여기서 챙긴다**
    #     안 그러면 아무 종목이나 후보로 들어온다
    def 무리깊(x, 문):
        m = x.get("무리낙폭")
        return (m is not None and m <= 문
                and 재무통과(x) and 대금통과(x) and 크기통과(x))
    ...
    도전자 = [
        ("지금 규칙", 지금),
        ("① **무리와 같이 빠짐** (±10%p)",
         lambda x: 지금(x) and 무리있(x) and -10 <= x["무리대비"] <= 10),
        ("② **나만 빠진 것을 뺀다** (무리보다 -15%p↓ 제외)",
         lambda x: 지금(x) and 무리있(x) and x["무리대비"] > -15),
        ("③ 무리보다 **덜 빠졌다** (+3%p↑)",
         lambda x: 지금(x) and 무리있(x) and x["무리대비"] >= 3),
        ("④ **무리가 폭락한 날** (무리 중앙값 -20%↓)",
         lambda x: (지금(x) and x.get("무리낙폭") is not None
                    and x["무리낙폭"] <= -20)),
        ("⑤ ⭐ **테마형 무리 + 같이 빠짐**",
         lambda x: (지금(x) and 테마형(x) and 무리있(x)
                    and -10 <= x["무리대비"] <= 10)),
        ("⑥ [견줌] 업종형 무리 + 같이 빠짐",
         lambda x: (지금(x) and 업종형(x) and 무리있(x)
                    and -10 <= x["무리대비"] <= 10)),
        ("⑦ 테마형 무리 (같이 빠짐 조건 없이)",
         lambda x: 지금(x) and 테마형(x)),
        ("⑧ 무리에 든 것만 (견줌)",
         lambda x: 지금(x) and x.get("무리") is not None),

        # ══ ⭐⭐ 217차 — **더하기(OR)** 로 바꾼다 (2026-09-10) ══
        #  197차 도전자는 **전부 좁히기**였다 (지금 and 무리조건) -> 기회 2~8%
        #  214차에서 「기존 OR 시장낙폭≤-7%」가 +10.2%p · 기회 323% 였다.
        #  무리는 시장보다 **잘게** 나뉘니 더 자주 걸릴 수 있다
        ("⑧ ⭐⭐ 기존 **OR 무리낙폭**≤-10%",
         lambda x: 지금(x) or 무리깊(x, -10)),
        ("⑨ ⭐⭐ 기존 **OR 무리낙폭**≤-15%",
         lambda x: 지금(x) or 무리깊(x, -15)),
        ("⑩ ⭐⭐ 기존 **OR 무리낙폭**≤-20%",
         lambda x: 지금(x) or 무리깊(x, -20)),
        ("⑪ ⭐ 기존 OR 무리낙폭≤-15% **AND 나도 빠짐**",
         lambda x: (지금(x) or (무리깊(x, -15) and 있(x, "낙폭20")
                                and x["낙폭20"] <= -5))),
        ("⑫ ⭐ 기존 OR 무리낙폭≤-15% **AND 테마형**",
         lambda x: 지금(x) or (무리깊(x, -15) and 테마형(x))),
        ("⑬ ⭐ 기존 OR (무리낙폭≤-15% **AND 나만 더 빠짐**)",
         lambda x: (지금(x) or (무리깊(x, -15) and 있(x, "무리대비")
                                and x["무리대비"] <= -5))),
        ("⑭ ⭐ 기존 OR (무리낙폭≤-15% **AND 나는 덜 빠짐**)",
         lambda x: (지금(x) or (무리깊(x, -15) and 있(x, "무리대비")
                                and x["무리대비"] >= 0))),
        ("⑮ ⭐ 기존 OR 무리낙폭≤-15% **AND 회전율 3%↓**",
         lambda x: 지금(x) or (무리깊(x, -15) and _회전(x, 3))),
    ]
    ...
    print("\n" + "=" * 108)
    print("  197차 · **상관 무리의 「같이 빠짐」을 4관문에**")
    ...
    # ── 판정 ──
    print("\n" + "=" * 108)
    # ══ ⭐ 217차 — **기회 비율** (197차엔 없던 것) ══
    print("\n" + "=" * 108)
    print("  217차 · ⭐ **기회 비율** — 승률만 보면 「적게 사서 오른 것」을 통과시킨다")
    print("=" * 108)
    _기217 = 점수([x for x in 사건 if 지금(x)])
    print(f"\n  {'도전자':<44}{'건수':>10}{'1년에':>8}"
          f"{'이김':>8}{'기회':>8}{'차이':>8}")
    for 라, fn in 도전자:
        r = 점수([x for x in 사건 if fn(x)])
        if not r or not _기217:
            print(f"  {라:<44}{'표본 부족':>10}")
            continue
        print(f"  {라:<44}{r[2]:>10,}{r[2]/해수:>8.0f}{r[0]:>7.1f}%"
              f"{r[2]/_기217[2]*100:>7.0f}%{r[0]-_기217[0]:>+7.1f}p")
    print("\n     ⚠️ 기회 **100% 밑**이면 좁히기다 — 승률이 올라도 놓치는 게 는다")
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      os.environ.get("LAB_OUT") or "2026-09-10_197차_4관문_무리같이빠짐.txt")
