...
def _장1(q, 보유):
    ...
    if 상태 == "pre":
        ...
        리드 = ("08:55에 예상체결가를 보고 살지 정합니다. 그 전까지는 참고용 목록이고, "
              "아래 범위는 문턱가가 들어올 자리입니다.")
        라벨, 라벨색 = "아침에 볼 목록 · 아직 주문 아님", C["보"]
        각주 = (f"후보 {전체}종목 중 {n}개 · 열에 아홉 날 문턱가가 이 범위에 듭니다. "
              f"실제 값은 08:55에 하나로 정해집니다.")
    elif 상태 == "wait":
        킥, 제, 배, 배색 = "QUANT · 08:55 확정", "오늘 살 것: 없음", "안 산다", C["보"]
        ...
        킥, 제, 배, 배색 = ("QUANT · 08:55 확정", f"오늘 살 것: {n}개",
                        f"지정가 주문 {n}", C["빨"])
        리드 = (f"상대갭이 −{abs(R.상대갭문턱):g}%p 아래로 내려간 종목만 삽니다. "
              f"갭이 깊은 순으로 최대 {최대}종목입니다.")
        라벨, 라벨색 = "살 종목 · 지정가 주문 대상", C["빨"]
        앞, 뒤 = R.몫들[0], R.몫들[1]
        각주 = (f"{'반' if abs(앞[0] - .5) < 1e-9 else f'{앞[0] * 100:g}%'}은 +{앞[1]:g}%에, "
              f"{'반' if abs(뒤[0] - .5) < 1e-9 else f'{뒤[0] * 100:g}%'}은 +{뒤[1]:g}%에 파는 "
              "지정가를 같이 걸어 둡니다 · 09:01에 체결 안 된 주문은 반드시 취소하세요.")
    else:
        킥, 제, 배, 배색 = ("QUANT · 08:55 확정" if 확정 else "QUANT · 08:00 후보",
                        "오늘 볼 것 없음", "후보 0", C["보"])
        리드, 라벨, 라벨색 = "", "", C["보"]
    ...
    if 팔것 or 뒤팔것:
        ...
        if 팔것:
            말.append(f"오늘 반만 정리: {' · '.join(_esc(x.get('이름', '')) for x in 팔것)} · "
                     f"{R.앞몫기한}거래일이 지났습니다. 앞의 반만 정리하고 나머지 반은 "
                     f"+{R.뒷몫목표:g}%까지 기다립니다.")
        if 뒤팔것:
...
def _장3():
    옛낙, 새낙 = _계좌낙폭()
    앞, 뒤 = R.몫들[0], R.몫들[1]
    앞말 = "반" if abs(앞[0] - .5) < 1e-9 else f"{앞[0] * 100:g}%"
    뒤말 = "나머지 반" if abs(뒤[0] - .5) < 1e-9 else f"나머지 {뒤[0] * 100:g}%"
    앞말2 = "앞의 반" if abs(앞[0] - .5) < 1e-9 else f"앞의 {앞[0] * 100:g}%"
    뒤말2 = "뒤의 반" if abs(뒤[0] - .5) < 1e-9 else f"뒤의 {뒤[0] * 100:g}%"
    블록 = (
        ("어떻게 사나",
         [f"08:55에 5분, 여기서 살지 정해진다. 후보들의 예상체결가를 보고, 그 값들의 중앙값보다 "
          f"{abs(R.상대갭문턱):g}%p 더 빠진 것만 최대 {R.하루최대종목}종목 지정가로 삽니다.",
          "09:01에 체결 안 된 주문은 반드시 취소하세요. 맞는 게 없으면 아무것도 사지 않습니다."]),
        ("목표에 닿으면 · 둘로 나눠 판다",
         [f"한 종목을 사면 주문을 둘로 나눠 겁니다. 산 주식의 {앞말}은 +{앞[1]:g}%에 팔아 이익을 "
          f"일찍 챙기고, {뒤말}은 +{뒤[1]:g}%까지 기다립니다.",
          f"나눠 팔면 계좌 흔들림이 {abs(옛낙):.1f}% → {abs(새낙):.1f}%로 줄었습니다."]),
        ("목표에 안 닿으면 · 날짜로 끝낸다",
         [f"{앞말2}은 {앞[2]}거래일, {뒤말2}은 {뒤[2]}거래일이 지나면 그날 값으로 정리합니다.",
    ...
    행 = "".join(
        ...
        + "".join(_본문(_괄호nowrap(_굵(x))) for x in 줄들) + '</div></div>'
        for 라, 줄들 in 블록)
