#!/usr/bin/env python3
r"""
solo_lab.py — **144차 · 재료 하나를 축으로 규칙을 짠다** (2026-09-08 신설)

## 사용자 지적
```
「단독 재료 테스트나 새로운 조합 테스트는 진행했어?」
```
**안 했다.** 어제 계획해놓고 오늘 돌린 141·142·143차는 전부 **우리 재료**
(갭·낙폭·볼린저·비중)였다. 단독으로 돌린 재료는 공시(133차)와
122차의 넷(갭상승·거래대금·리포트·갭하락)뿐이다

## 안 해본 재료 (전부 **얹기**로만 봤다)
```
수급        127차 얹기 · 128차 고르기 -> 단독 없음
섹터 상대강도  129차 얹기 -> 단독 없음
컨센서스      106차 얹기 -> 단독 없음
임원·대주주   **아예 안 함**
증자·감자     97d 얹기 -> 단독 없음
```
...
## 짜임새 — 재료를 **축**으로
```
축      그 재료가 뛰어난 종목을 **매일 상위 N개** 골라 다음날 시가에 산다
        (우리 조건은 안 건다 — 볼린저·낙폭·갭 전부 뺀다)
층      ① 재료만  ② +재무  ③ +시총 500~2,000억  ④ +거래대금 1억  ⑤ 전부
매도    124차 나눠팔기 (반 +15%/40일 · 반 +40%/90일)
견줌    우리 규칙 = 6,432만 · 낙폭 -3.3% · 103건
```
⚠️ **단독으로 져도 버리지 않는다.** 우리 규칙의 재료도 단독으로는 전부 진다
   (갭 하락 -3.5% 단독 = 낙폭 -99%). 단독 시험은 **거르는 도구가 아니라
   이해하는 도구**다 — 그 재료가 무엇을 하는지 알기 위한 것이다

쓰는 법:
    python scripts\solo_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    for i, d1 in enumerate(날):
        ...
        for code, v in 주가[d1].items():
            ...
            # ⚠️⚠️ **시총은 「신호」가 아니라 「우주」다** (2026-09-08 고침).
            #    처음엔 「재료가 축이니 우리 조건을 다 뺀다」며 시총까지 풀었는데
            #    후보가 수백만 건으로 불어나 **메모리가 터져 죽었다.**
            #    우주(살 수 있는 범위)는 고정하고 **신호만** 뺀다
            if 시총 < 5e10 or 시총 >= 2e11:
                continue
            fm = 재무값(code, d1)
            ...
            볼 = (c1 - s20) / (2 * sd)
            # ⚠️ **볼린저·낙폭 문턱을 걸지 않는다** — 재료가 축이기 때문이다
            if sq[kk - 20] <= 0:
                continue
            낙 = (c1 / sq[kk - 20] - 1) * 100
    ...
    기본틀["대금하한"] = 0.0
    기본틀["볼린저"], 기본틀["낙폭20"] = 9e9, 9e9
    기본틀["상대갭"] = 99.0          # 갭 문턱을 끈다

    def 있(x, k):
        return x.get(k) is not None

    재료들 = [
        ("외국인 20일 순매수 많은 순", "외인20", True),
        ("외국인 20일 순매수 적은 순", "외인20", False),
        ("외국인 5일 순매수 많은 순", "외인5", True),
        ("기관 20일 순매수 많은 순", "기관20", True),
        ("기관 5일 순매수 많은 순", "기관5", True),
        ("개인 5일 순매수 적은 순", "개인5", False),
        ("외국인 지분율 20일 상승 큰 순", "지분20변화", True),
        ("섹터 대비 더 빠진 순", "섹터대비", False),
        ("섹터 대비 덜 빠진 순", "섹터대비", True),
        ("섹터가 많이 빠진 순", "섹터낙폭", False),
        ("시장 대비 더 빠진 순", "상대강도", False),
        ("회전율 높은 순", "회전율", True),
        ("잉여금비율 높은 순", "잉여금", True),
        ("ROE 높은 순", "ROE", True),
        ("영업이익률 높은 순", "영업이익률", True),
        ("부채비율 낮은 순", "부채", False),
    ]

    층들 = [
        ("① 재료만", {}),
        ("② +재무", {"거름": lambda x: (x.get("잉여금") or -9e9) >= 30
                     and (x.get("부채") or 9e9) <= 80}),
        ("③ +거래대금 0.5억↑", {"대금하한": 0.5}),
        ("④ +거래대금 1억↑", {"대금하한": 1.0}),
        ("⑤ 전부", {"시총상한": 2000, "대금하한": 1.0,
                    "거름": lambda x: (x.get("잉여금") or -9e9) >= 30
                    and (x.get("부채") or 9e9) <= 80}),
    ]

    print("=" * 104)
    print("  144차 · 재료 하나를 **축으로** 규칙을 짠다 (우리 조건 없이)")
    print(f"  후보 {len(사건):,}건 · 매도는 124차 나눠팔기")
    print("  ⚠️ 견줄 것: 우리 규칙 = 6,432만 · 낙폭 -3.3% · 103건")
    print("=" * 104)

    for 라, 키, 뒤집 in 재료들:
        n2 = sum(1 for x in 사건 if 있(x, 키))
        if n2 < 5000:
            print(f"\n  ── {라} ── 표본 {n2:,}건 — **부족**")
            continue
        print(f"\n  ── {라} ── (그 값이 있는 후보 {n2:,}건)")
        ...
        for 층라, 바꿀 in 층들:
            ...
            # 그 값이 없는 것은 아예 뺀다
            앞거름 = c.get("거름")
            c["거름"] = (lambda x, f=앞거름, k=키:
                         있(x, k) and (f(x) if f else True))
            표(시뮬(c), 층라)
    print("")
