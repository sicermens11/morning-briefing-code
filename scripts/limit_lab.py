#!/usr/bin/env python3
r"""
limit_lab.py — **151차 · 예상체결가에 지정가로 걸면** (2026-09-08 신설)

## 왜 — 지금 시뮬이 실제보다 **낙관적이다**
```
지금 시뮬   09:00 시가에 **무조건** 산다
실제 절차   08:50 예상체결가에 **지정가**를 건다
           -> 09:00 시가가 그보다 **높으면 안 사진다**
```
예상체결가는 위아래로 비슷하게 틀리므로, **딱 맞춰 걸면 절반쯤만 사진다.**
1년에 10번 사던 게 **5번**이 될 수 있다. 그냥 넘어갈 수 없는 차이다

## 어떻게 흉내내나
```
과거 자료에 **08:50 예상체결가가 없다.** 그래서:
   예상체결가 = 실제 시가 × (1 + 오차)     오차 ~ 정규분포(0, σ)
   지정가     = 예상체결가 × (1 + 여유)
   체결 조건   시가 <= 지정가
σ 는 109차 오차 시뮬에서 쓴 ±0.3 / 0.5 / 1.0%p 를 그대로 쓴다
⚠️ **실측은 오늘부터 쌓인다** (gap_error.py · 지금 0건).
   20건 모이면 진짜 값으로 다시 돌린다
```

## 재는 것
```
A 여유 0%       예상체결가에 딱 맞춰 → 체결률과 성적
B 여유 +0.2 / +0.5 / +1.0 / +2.0%   → 체결률은 오르고 조금 비싸게 산다
C 시장가 (무조건 체결)              → 지금 시뮬과 같다
D ⭐ 안 사진 것들은 그 뒤에 어땠나
   올랐으면 **놓친 것**, 내렸으면 **피한 것**
   우리 규칙은 「싸게 열리는 걸 산다」니 **비싸게 열려 안 사진 건 잘된 것**일 수도
```

쓰는 법:
    python scripts\limit_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    print("=" * 104)
    print("  151차 · 예상체결가에 **지정가**로 걸면")
    print("  (지금 시뮬은 「09:00 시가에 무조건 산다」 — 실제보다 낙관적이다)")
    print("=" * 104)
    ...
    for 오차폭 in (0.3, 0.5, 1.0):
        ...
        print("=" * 104)
        print(f"  {'거는 법':<26}{'끝 자산':>15}{'연평균':>9}{'낙폭':>8}"
              f"{'산 것':>7}{'못 산 것':>9}{'체결률':>8}{'돈÷낙폭':>9}")
        기 = 시뮬(기본틀)
        점 = 기["연"] / max(abs(기["낙"]), 3.0)
        print(f"  {'시장가 (무조건 체결)':<26}{기['끝']:>15,.0f}"
              f"{기['연']:>8.2f}%{기['낙']:>7.1f}%{기['산']:>7}"
              f"{0:>9}{100.0:>7.1f}%{점:>9.2f}")
        for 여유 in (0.0, 0.2, 0.5, 1.0, 2.0):
            c = dict(기본틀)
            c["여유"], c["오차폭"] = 여유, 오차폭
            r = 시뮬(c)
            총 = r["산"] + r["미체결"]
            체 = (r["산"] / 총 * 100) if 총 else 0
            점2 = r["연"] / max(abs(r["낙"]), 3.0)
            라 = ("예상체결가 그대로" if 여유 == 0
                  else f"예상체결가 +{여유}%")
            print(f"  {라:<26}{r['끝']:>15,.0f}{r['연']:>8.2f}%"
                  f"{r['낙']:>7.1f}%{r['산']:>7}{r['미체결']:>9}"
                  f"{체:>7.1f}%{점2:>9.2f}")

    # ── D 안 사진 것들은 어땠나 ──
    print("\n" + "=" * 104)
    print("  D ⭐ 안 사진 것들은 그 뒤에 어땠나 (오차 ±0.5%p · 여유 0%)")
    print("=" * 104)
    c = dict(기본틀)
    c["여유"], c["오차폭"] = 0.0, 0.5
    r = 시뮬(c)
    ...
    print(f"\n  못 산 것 {len(못산):,}건")
    if 못산:
        print(f"  {'기간':<8}{'못 산 것':>12}{'산 것(견줌)':>16}{'차이':>10}")
        산것 = []
        c2 = dict(기본틀)
        # 산 것은 전체 후보 중 문턱을 넘은 것 — 견줌용으로 전부
        for 기간 in (20, 40, 90):
            a = [앞수익(x, 기간) for x in 못산]
            a = [v for v in a if v is not None]
            if len(a) < 10:
                print(f"  {기간}일{'':<4}표본 부족 ({len(a)})")
                continue
            평a = sum(a) / len(a)
            print(f"  {기간}일{'':<4}{평a:>12.2f}%{'':>16}")
        print("\n  ⇒ 못 산 것이 **올랐으면 놓친 것**, **내렸으면 피한 것**이다")
        print("     우리 규칙은 「싸게 열리는 걸 산다」니")
        print("     비싸게 열려 안 사진 건 **잘된 것**일 수 있다")
    print("")
...
if __name__ == "__main__":
    ...
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      "data", "_labs",
                      os.environ.get("LAB_OUT") or "2026-09-08_151차_지정가매수.txt")
