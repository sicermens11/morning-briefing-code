#!/usr/bin/env python3
r"""
bias_lab.py — **183차 · 자료가 대형주에만 쏠려 있나** (2026-09-09 신설)

## 왜 — 가장 나쁜 것을 찾았다
```
수급(127·128차) · 공시(121·132·133차) · 컨센서스(168차) 는
**대형주에만 자료가 있다.**
우리 우주는 300~2,000억 소형주인데,
「수급은 안 통한다」 「공시는 안 통한다」 는 결론이
**소형주에 대한 결론이 아닐 수 있다**
(168차 실측: 컨센서스가 붙은 후보는 1년 77개 = 후보의 **3%**)
```
⚠️ 이건 **180차(무리별 규칙) 성패와 무관하다.**
   자료가 한쪽에 쏠린 것은 그 자체로 문제다

## 재는 것
```
A 덮는 범위 x 규모대   재료마다 **규모대별로 몇 %에 붙나**
                    -> 3% 밖에 안 붙으면 그 재료의 결론은 못 믿는다
B 재료 효과 x 규모대   **자료가 있는 것끼리만** 견준다
                    (없는 것을 「없음」으로 세면 결론이 흐려진다)
C 업종별 덮는 범위     업종마다도 쏠렸나
D ⭐ 자료 있는 것만 vs 없는 것만
                    「자료가 있다」는 사실 자체가 신호일 수 있다
```
...
def main():
    ...
    규모대 = (("소형 300~500억", 300, 500), ("중소 500~800억", 500, 800),
              ("중형 800~1,200억", 800, 1200),
              ("대형 1,200~2,000억", 1200, 2000),
              ("초대형 2,000억↑", 2000, 9e9))

    재료들 = (("컨센서스(90일)", lambda x: (x.get("컨_90") or 0) > 0),
              ("임원매매(90일)", lambda x: (x.get("임_90") or 0) != 0),
              ("뉴스(20일)", lambda x: (x.get("뉴_20") or 0) > 0),
              ("그날 공시", lambda x: x.get("공시있나") == 1.0),
              ("수급 외인20", lambda x: x.get("외인20") is not None),
              ("수급 기관20", lambda x: x.get("기관20") is not None))

    print("\n" + "=" * 118)
    print("  183차 · **자료가 대형주에만 쏠려 있나**")
    print("  ⚠️ 「수급은 안 통한다」 「공시는 안 통한다」는 결론이")
    print("     **소형주에 대한 결론이 아닐 수 있다**")
    print(f"  후보 {len(사건):,}건 · 바탕 20일 43.5% · +0.33%")
    print("=" * 118)

    # ── A 덮는 범위 x 규모대 ──
    print("\n  ── A **재료가 규모대별로 몇 %에 붙나** ──")
    머3 = f"  {'재료':<18}" + "".join(f"{라.split()[0]:>12}" for 라, _, _ in 규모대)
    print(머3)
    규모칸 = {}
    for 라, lo, hi in 규모대:
        규모칸[라] = [x for x in 사건 if lo <= x["시총억"] < hi]
    for 재라, fn in 재료들:
        줄 = f"  {재라:<18}"
        for 라, _, _ in 규모대:
            칸 = 규모칸[라]
            n = sum(1 for x in 칸 if fn(x))
            줄 += f"{(n/len(칸)*100 if 칸 else 0):>11.1f}%"
        print(줄)
    print("  ⚠️ **10% 아래면 그 재료의 결론은 그 규모대에서 못 믿는다**")

    print(f"\n  {'규모대':<18}{'사건 수':>12}{'종목 수':>10}")
    for 라, _, _ in 규모대:
        칸 = 규모칸[라]
        print(f"  {라:<18}{len(칸):>12,}{len({x['code'] for x in 칸}):>10,}")

    # ── B 재료 효과 x 규모대 ──
    def 세기(칸, 최소=100):
        v = [x["_20"] for x in 칸 if x.get("_20") is not None]
        if len(v) < 최소:
            return None
    ...
    def 신호(x):
        return (x.get("재통과") == 1.0 and x["대금억"] >= 1.0
                and x["볼린저"] <= -1.0 and x["낙폭20"] <= -10.0)

    print("\n  ── B **자료가 있는 것끼리만** 견주면 (규모대별) ──")
    print("     ⚠️ 자료 없는 것을 「없음」으로 세면 결론이 흐려진다")
    for 재라, fn in 재료들:
        print(f"\n   [{재라}]")
        print(f"  {'규모대':<18}{'있음 건수':>10}{'이김':>8}{'평균':>9}"
              f"{'없음 건수':>10}{'이김':>8}{'평균':>9}{'차이':>8}")
        for 라, lo, hi in 규모대:
            칸 = [x for x in 규모칸[라] if 신호(x)]
            있 = 세기([x for x in 칸 if fn(x)])
            없 = 세기([x for x in 칸 if not fn(x)])
            if not 있 and not 없:
                continue
            a = f"{있[2]:>10,}{있[0]:>7.1f}%{있[1]:>+9.2f}" if 있 else f"{'표본부족':>27}"
            b = f"{없[2]:>10,}{없[0]:>7.1f}%{없[1]:>+9.2f}" if 없 else f"{'표본부족':>27}"
            c = f"{있[0]-없[0]:>+8.1f}p" if (있 and 없) else ""
            print(f"  {라:<18}{a}{b}{c}")

    # ── C 업종별 덮는 범위 ──
    print("\n  ── C **업종별로도 쏠렸나** (건수 많은 업종 12개) ──")
    업칸 = {}
    for x in 사건:
        업 = _업종.get(x["code"])
        if 업:
            업칸.setdefault(업, []).append(x)
    큰업 = [u for u in sorted(업칸, key=lambda z: -len(업칸[z]))][:12]
    print(f"  {'업종':<16}{'사건':>10}" + "".join(
        f"{재라[:8]:>11}" for 재라, _ in 재료들))
    for 업 in 큰업:
        칸 = 업칸[업]
        줄 = f"  {업[:15]:<16}{len(칸):>10,}"
        for _재라, fn in 재료들:
            줄 += f"{sum(1 for x in 칸 if fn(x))/len(칸)*100:>10.1f}%"
        print(줄)

    # ── D 자료가 있다는 것 자체가 신호인가 ──
    print("\n  ── D ⭐ **「자료가 있다」는 사실 자체가 신호인가** ──")
    print(f"  {'설정':<34}{'건수':>9}{'1년에':>7}{'20일 이김':>10}{'평균':>9}")
    바 = [x for x in 사건 if 신호(x)]
    for 라, 칸 in ([("지금 규칙 (전부)", 바)]
                   + [(f"+ {재라} 있음", [x for x in 바 if fn(x)])
                      for 재라, fn in 재료들]
                   + [(f"+ {재라} 없음", [x for x in 바 if not fn(x)])
                      for 재라, fn in 재료들]):
        p = 세기(칸)
        if p:
            print(f"  {라:<34}{p[2]:>9,}{p[2]/해수:>7.0f}"
                  f"{p[0]:>9.1f}%{p[1]:>+9.2f}")

    print("\n" + "=" * 118)
    print("  읽는 법")
    print("    - **A 가 이 시험의 핵심이다.** 10% 아래면 그 재료의 옛 결론은 버린다")
    print("    - B 에서 규모대마다 부호가 갈리면 **규모별로 따로 써야** 한다")
    print("    - D 에서 「자료 있음」이 그것만으로 좋으면")
    print("      그건 재료가 아니라 **「덩치가 크다」는 대리 신호**일 수 있다")
    print("=" * 118)
    return 0
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      os.environ.get("LAB_OUT") or "2026-09-09_183차_자료쏠림.txt")
