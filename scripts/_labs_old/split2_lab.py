#!/usr/bin/env python3
r"""
split2_lab.py — **절반은 +20%에, 절반은 +40%까지** (2026-09-07 신설)

## 왜 (사용자 질문)
```
「지금이 더 나은 것 같긴 한데, 바꾸면 종목 추천이 더 많아지나?
  둘 다 병행할 순 없는 거야?」

먼저 사실:  매수 조건은 **똑같다.** 파는 것만 다르다.
           그래서 추천이 늘지 않는다 — 오히려 자금이 묶여 **100번 -> 87번**으로 준다

병행:      한 종목을 사서 **주수를 반으로 나눠**
           절반은 +20% 지정가 · 40일 / 절반은 +40% 지정가 · 90일
⚠️ 83차 part_lab 은 **아래로** 나누는 것이었다 (+10%에 절반) -> 「개선 없음」
   여기는 **위로** 나눈다. 방향이 달라 안 재봤다
```
## 재는 것
```
A 지금 (20% · 40일 전부)
B 바꾸면 (40% · 90일 전부)
C ⭐ 절반씩 나누기 — 비율 50:50 / 70:30 / 30:70
D 목표 조합도 바꿔서 (20+30 / 20+40 / 15+40)
```
⚠️ 나눠도 **한 종목에 들어가는 총액은 같다** (자산 20%). 주수만 반으로 갈린다

쓰는 법:
    python scripts\split2_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    for 끝년, 라 in ((None, "전체 기간"), ("2024", "2025·26 제외")):
        ...
        기 = 시뮬(확정, 끝년=끝년)
        표(기, "A 지금 (20% · 40일)")
        표(시뮬({**확정, "목표": 40, "최대보유": 90}, 끝년=끝년),
          "B 바꾸면 (40% · 90일)", 기)

        print("")
        print("    -- C ⭐ 절반씩 나누기 (20%·40일 + 40%·90일) --")
        for 앞, 뒤 in ((0.5, 0.5), (0.7, 0.3), (0.3, 0.7)):
            c = dict(확정)
            c["나눔"] = ((앞, 20, 40), (뒤, 40, 90))
            표(시뮬(c, 끝년=끝년),
              "나눔 " + str(int(앞 * 100)) + ":" + str(int(뒤 * 100)), 기)

        print("")
        print("    -- D 목표 조합을 바꿔서 (50:50) --")
        for 목a, 보a, 목b, 보b in ((20, 40, 30, 60), (20, 40, 30, 90),
                                    (15, 40, 40, 90), (20, 60, 40, 90),
                                    (25, 60, 50, 90)):
            c = dict(확정)
            c["나눔"] = ((0.5, 목a, 보a), (0.5, 목b, 보b))
            표(시뮬(c, 끝년=끝년),
              str(목a) + "%·" + str(보a) + "일 + " + str(목b) + "%·"
              + str(보b) + "일", 기)
        print("")
