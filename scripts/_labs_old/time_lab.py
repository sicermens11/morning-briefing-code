#!/usr/bin/env python3
r"""
time_lab.py — **132차 · 공시가 「언제」 떴나** (2026-09-07 신설)

## 사용자 질문에서 나왔다
```
「뉴스는 언제 나왔냐가 중요하지 않아?」  → 맞다. 그리고 그건 공시도 마찬가지다
```
우리 절차와 정확히 맞물린다:
```
d1 15:30  장이 닫힌다. 이때 종가로 후보를 뽑는다
d1 16:00 ~ d2 09:00   ← **이 창에 뜬 공시는 아직 주가에 안 들어갔다**
d2 09:00  우리가 산다
```
장중에 뜬 공시는 이미 그날 주가에 반영됐다. **창 안의 것만이 미반영 정보**다.

## 왜 뉴스가 아니라 공시로 먼저 하나
```
뉴스   2025-09-03 ~ 2026-09-07 = **1년치**. 시각은 100% 붙어 있다(12만 건).
       그런데 우리 규칙은 1년에 실제 매수가 **10건 안팎**이다
       -> 자본 시뮬로 잴 표본이 안 나온다. **1년 더 쌓아야 한다**
공시   kind-time 4,105일 = **16.7년치**. 시각이 있다 (18:32 꼴)
       -> 같은 질문을 **제대로 잴 수 있다**
```
⚠️ 잇는 법: kind-time 공시번호 `20260904000786` 와
   dart-daily 접수번호 `20260904900786` 는 **뒤 5자리로 붙는다**
   (실측 매칭률 78~100%. 통째 매칭은 절반도 안 붙는다)

## 재는 것
```
A 창에 공시가 있었나 / 없었나
B 창의 공시가 몇 건인가
C 시간대   장후(16~24) · 새벽(00~08) · 장전(08~09)
D 종류     「챙길공시」인가 그밖인가
```
거름(조건)과 고르기(순서) 둘 다로 잰다

쓰는 법:
    python scripts\time_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    for i, d1 in enumerate(날):
        ...
        for code, v in 주가[d1].items():
            ...
            사건.append({
                ...
                "갭": g, "매수": 매수})
    # ══ 공시 시각 붙이기 ══ (132차)
    print("  공시·시각 읽는 중...", flush=True)

    def _분(시문자):
        """'18:32' -> 1112 (분). 못 읽으면 None"""
        s = str(시문자 or "").strip()
        if ":" in s:
            a, _, b = s.partition(":")
    ...
    # 날짜 -> {종목코드: [(분, 챙길것인가), ...]}
    공시시각 = {}
    _kd = os.path.join(O._DATA, "kind-time")
    _dd = os.path.join(O._DATA, "dart-daily")
    붙은날, 빈날 = 0, 0
    for d3 in 날:
        kp = os.path.join(_kd, d3 + ".json")
        dp = os.path.join(_dd, d3 + ".json")
        if not (os.path.exists(kp) and os.path.exists(dp)):
            빈날 += 1
            continue
        try:
            시각표 = (json.load(io.open(kp, encoding="utf-8-sig"))
                      .get("시각") or {})
            dj = json.load(io.open(dp, encoding="utf-8-sig"))
        except ValueError:
            빈날 += 1
            continue
        # ⚠️ **뒤 5자리로 잇는다** (실측). 통째로는 절반도 안 붙는다
        뒤5 = {}
        for 번, 시 in 시각표.items():
            if len(str(번)) >= 5:
                뒤5[str(번)[-5:]] = _분(시)
        하루 = {}
        for 챙길, 목록 in ((True, dj.get("챙길공시") or []),
            ...
            for x3 in 목록:
                c3 = str(x3.get("종목코드") or "")
                if not c3:
                    continue
                분 = 뒤5.get(str(x3.get("접수번호", ""))[-5:])
                if 분 is None:
                    continue
                하루.setdefault(c3, []).append((분, 챙길))
        if 하루:
            공시시각[d3] = 하루
            붙은날 += 1
    print(f"    공시 시각 붙은 날 {붙은날:,} · 못 붙은 날 {빈날:,}", flush=True)

    # ── 사건마다 「d1 16:00 ~ d2 09:00」 창을 본다 ──
    _창시작 = 16 * 60      # d1 16:00
    _창끝 = 9 * 60         # d2 09:00
    있음 = 0
    for x in 사건:
        i3 = x["인"] - 1
        d1v, d2v = 날[i3], 날[x["인"]]
        늦 = [(분, 챙) for 분, 챙 in ((공시시각.get(d1v) or {}).get(x["code"]) or [])
              if 분 is not None and 분 >= _창시작]
        이 = [(분, 챙) for 분, 챙 in ((공시시각.get(d2v) or {}).get(x["code"]) or [])
              if 분 is not None and 분 < _창끝]
        창 = 늦 + 이
        x["창공시수"] = len(창)
        x["창챙길수"] = sum(1 for _, 챙 in 창 if 챙)
        x["창장후"] = sum(1 for 분, _ in 늦 if 분 >= _창시작)
        x["창새벽"] = sum(1 for 분, _ in 이 if 분 < 8 * 60)
        x["창장전"] = sum(1 for 분, _ in 이 if 8 * 60 <= 분 < _창끝)
        # 장중에 뜬 것 (이미 주가에 반영됐다 — 견줄 대상)
        x["장중공시수"] = sum(
            1 for 분, _ in ((공시시각.get(d1v) or {}).get(x["code"]) or [])
            if 분 is not None and 9 * 60 <= 분 < _창시작)
        if 창:
            있음 += 1
    print(f"  창(전날 16시~당일 9시)에 공시가 있는 사건 "
          f"**{있음:,}/{len(사건):,}** ({있음/max(len(사건),1)*100:.0f}%)",
          flush=True)
    ...
    def 값(k, 뒤집기=False):
        def _키(x):
            ...
            if v is None:
                return (1, 0.0)
            return (0, -v if 뒤집기 else v)
        return _키

    거름들 = [
        ("── A 창에 공시가 있었나 (전날 16시 ~ 당일 9시) ──", None),
        ("공시가 **있었다**", lambda x: (x.get("창공시수") or 0) > 0),
        ("공시가 **없었다**", lambda x: (x.get("창공시수") or 0) == 0),
        ("── B 몇 건인가 ──", None),
        ("1건", lambda x: (x.get("창공시수") or 0) == 1),
        ("2건 이상", lambda x: (x.get("창공시수") or 0) >= 2),
        ("── C 시간대 ──", None),
        ("**장후**(16~24시)에 떴다", lambda x: (x.get("창장후") or 0) > 0),
        ("**새벽**(00~08시)에 떴다", lambda x: (x.get("창새벽") or 0) > 0),
        ("**장전**(08~09시)에 떴다", lambda x: (x.get("창장전") or 0) > 0),
        ("── D 종류 · 견줄 것 ──", None),
        ("「챙길공시」가 있었다", lambda x: (x.get("창챙길수") or 0) > 0),
        ("그밖의 공시만 있었다",
         lambda x: (x.get("창공시수") or 0) > 0 and (x.get("창챙길수") or 0) == 0),
        ("(견줌) **장중**에 공시가 떴다 — 이미 반영된 것",
         lambda x: (x.get("장중공시수") or 0) > 0),
    ]

    고르기들 = [
        ("── 고르는 순서만 바꾼다 ──", None),
        ("창 공시가 **많은** 순", 값("창공시수", True)),
        ("창 공시가 **적은** 순", 값("창공시수")),
        ("「챙길공시」가 **많은** 순", 값("창챙길수", True)),
        ("장후 공시가 **많은** 순", 값("창장후", True)),
        ("(견줌) 낙폭 깊은 순", 값("낙폭20")),
    ]
