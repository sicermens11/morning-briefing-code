#!/usr/bin/env python3
r"""
flow2_lab.py — **수급을 「고르는 데」 써본다** (2026-09-07 신설)

## 왜 또 하나 만들었나
```
flow_lab 에서 수급 조건 18개가 **전부 졌다.**
그런데 진 이유가 「수급이 쓸모없어서」인지
**「조건을 걸면 후보가 줄어서」**인지 구별이 안 된다 —
   산 것이 103건 -> 76·78·26·11건으로 줄었다.
   「후보가 적으면 진다」는 건 이미 아는 사실이다(원리 ①)
```
⇒ **후보 수를 그대로 두고** 수급을 쓰는 법이 있다:
```
지금  갭이 제일 많이 벌어진 순으로 4종목을 고른다
바꿈  갭 문턱(-3.5%p)은 그대로 통과시키되,
      그 안에서 **수급이 좋은 순으로** 4종목을 고른다
```
후보 수·매수 횟수가 거의 같으므로, 차이가 나면 그건 **수급의 정보값**이다.
[[dont-add-conditions]] 의 요소분해와 같은 발상이다

⚠️ 이겨도 바로 안 바꾼다 — 4관문 + 오차 시뮬을 다시 걸어야 한다

쓰는 법:
    python scripts\flow2_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    def 값(k, 뒤집기=False):
        """그 값이 없는 것은 **맨 뒤**로 보낸다 (없다고 먼저 사지 않는다)"""
        def _키(x):
            v = x.get(k)
            if v is None:
                return (1, 0.0)
    ...
    고르기들 = [
        ...
        ("갭 큰 순 (지금)", None),
        ("── 외국인 ──", None),
        ("외국인 20일 순매수 **많은** 순", 값("외인20", True)),
        ("외국인 20일 순매수 **적은** 순", 값("외인20")),
        ("외국인 5일 순매수 **많은** 순", 값("외인5", True)),
        ("외국인 5일 순매수 **적은** 순", 값("외인5")),
        ("외국인 그날 순매수 **많은** 순", 값("외인1", True)),
        ("── 기관 ──", None),
        ("기관 20일 순매수 **많은** 순", 값("기관20", True)),
        ("기관 20일 순매수 **적은** 순", 값("기관20")),
        ("기관 5일 순매수 **많은** 순", 값("기관5", True)),
        ("── 개인 ──", None),
        ("개인 5일 순매수 **적은** 순", 값("개인5")),
        ("개인 5일 순매수 **많은** 순", 값("개인5", True)),
        ("── 지분율 ──", None),
        ("외국인 지분율 **높은** 순", 값("지분율", True)),
        ("외국인 지분율 **낮은** 순", 값("지분율")),
        ("지분율 20일 **많이 오른** 순", 값("지분20", True)),
        ("지분율 20일 **많이 내린** 순", 값("지분20")),
    ]
