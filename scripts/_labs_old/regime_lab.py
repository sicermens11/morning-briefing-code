#!/usr/bin/env python3
r"""
regime_lab.py — **141차 · 시장이 빠진 날 더 산다** (2026-09-08 신설)

## 왜
```
140차에서 스물여덟 지표 중 **시장 낙폭이 압도적 1위**였다:
   시장이 크게 빠진 날 산 것   40일 뒤 평균 **+24.4%**
   시장이 안 빠진 날 산 것                 **-0.4%**
그런데 129차에서 이걸 **조건으로 걸었을 때는 전패**였다
```
⚠️ **거르는 방식이 틀린 것이지 신호가 없는 게 아니다.**
```
조건으로 걸면  후보·매수가 줄고, 폭락장은 드물고,
              그때는 이미 계좌가 깎여 있어 넣을 돈도 적다
그 대신        **거르지 말고 「얼마나 넣을까」를 바꾼다** ← 한 번도 안 해봤다
```

## 재는 것
```
사는 조건은 **그대로**다 (소형 · 볼린저 -1.0 · 낙폭 -10 · 갭 -3.5 · 나눠팔기)
바꾸는 것은 **비중**뿐:
   평소            자산의 20%
   시장이 빠진 날    25% · 30% · 40% 로 키운다
   시장이 오른 날    15% · 10% 로 줄인다 (반대쪽도 본다)
사다리를 여러 벌 만들어 견준다. 하루 종목 수(4개)도 같이 바꿔본다
```
⚠️ 시장 낙폭은 **코스닥 지수 20일**로 잰다 (우리 후보 대부분이 코스닥이다)
⚠️ 비중을 키우면 낙폭도 커진다. **돈÷낙폭**으로 본다

쓰는 법:
    python scripts\regime_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    def 시뮬(c, 끝년=None, 시드=None, 시작년=None, 오차=0.0):
        ...
        for i in range(시작i, len(날)):
            ...
            for x in 골[:오늘상한]:
                ...
                # ⚠️ **오늘 비중**은 시장 국면에 따라 달라질 수 있다 (141차)
                쓸 = min(평 * 오늘비중, 현금, x["대금"] * 0.01)
                총주수 = int(쓸 // x["원시"])
    ...
    # 코스닥 20일 낙폭이 얼마나 자주 어디쯤인지 먼저 본다
    낙칸 = []
    for i2 in range(260, len(날)):
        v = 낙폭("코스닥", i2)
        if v is not None:
            낙칸.append(v)
    낙칸.sort()
    ...
    print("=" * 100)
    print("  141차 · 시장이 빠진 날 더 산다  (거르지 않고 **비중**만 바꾼다)")
    print("=" * 100)
    print(f"\n  코스닥 20일 낙폭 분포 ({len(낙칸):,}일)")
    print(f"    맨 아래 5%  {분위(0.05):>7.1f}%   10%  {분위(0.10):>7.1f}%"
          f"   25%  {분위(0.25):>7.1f}%")
    print(f"    가운데      {분위(0.50):>7.1f}%   위 25% {분위(0.75):>7.1f}%"
          f"   위 10% {분위(0.90):>7.1f}%")

    사다리들 = [
        ("평소 20% (지금)", None),
        ("-5%↓ 25% / 그밖 20%", [(-5, 0.25), (999, 0.20)]),
        ("-10%↓ 30% / -5%↓ 25% / 그밖 20%",
         [(-10, 0.30), (-5, 0.25), (999, 0.20)]),
        ("-10%↓ 40% / -5%↓ 30% / 그밖 20%",
         [(-10, 0.40), (-5, 0.30), (999, 0.20)]),
        ("-15%↓ 40% / -10%↓ 30% / -5%↓ 25% / 그밖 20%",
         [(-15, 0.40), (-10, 0.30), (-5, 0.25), (999, 0.20)]),
        ("-5%↓ 30% / 0%↓ 20% / 그밖 **10%**",
         [(-5, 0.30), (0, 0.20), (999, 0.10)]),
        ("(반대) -5%↓ 10% / 그밖 25%", [(-5, 0.10), (999, 0.25)]),
    ]
    상한들 = [
        ("하루 4종목 (지금)", None),
        ("-10%↓ 6종목 / 그밖 4종목", [(-10, 6), (999, 4)]),
        ("-10%↓ 8종목 / -5%↓ 6종목 / 그밖 4종목",
         [(-10, 8), (-5, 6), (999, 4)]),
    ]
    ...
    for 끝년, 라 in ((None, "전체 기간"), ("2024", "2025·26 제외")):
        ...
        print("\n    ── C 둘 다 ──")
        for 이름1, 사다리 in 사다리들[2:5]:
            for 이름2, 상한 in 상한들[1:]:
                c = dict(기본틀)
                c["사다리"], c["상한사다리"] = 사다리, 상한
                표(시뮬(c, 끝년=끝년),
                  f"{이름1.split(' /')[0]} + {이름2.split(' /')[0]}", 기)
        print("")
