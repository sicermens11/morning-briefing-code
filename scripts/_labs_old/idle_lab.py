#!/usr/bin/env python3
r"""
idle_lab.py — **143차 · 돈이 얼마나 놀고 있나** (2026-09-08 신설)

## 사용자 지적
```
「실제로 사는 날은 서른여섯 날에 한 번이면 매수 기회가 엄청 적은 건데?」
```
맞다. 그런데 **얼마나 적은지를 숫자로 안 재봤다.**
「36일에 한 번」은 **날짜**를 센 것이지 **돈이 얼마나 일하고 있나**가 아니다

## 재는 것
```
A 하루하루 **현금 비율**  — 자산 중 몇 %가 주식에 안 들어가 있나
B 한 해에 몇 번 사나
C 아무것도 안 들고 있는 날이 며칠인가
D 문턱을 낮추면 그게 얼마나 달라지나 (갭 -3.5 / -3.0 / -2.5)
```
⚠️ 현금이 놀면 그만큼 **기회비용**이다. 다만 122차에서 확인했듯이
   **자주 사면 비용이 삼킨다** — 10년에 107번 vs 4,521번 = 42배.
   그래서 「많이 사는 게 좋다」가 아니라 **「얼마나 노는지 알고 정하자」**는 것이다

쓰는 법:
    python scripts\idle_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    def 시뮬(c, 끝년=None, 시드=None, 시작년=None, 오차=0.0):
        ...
        return {"끝": 끝, "연": 연, "낙": 낙 * 100, "산": 산,
                "현금비율": (sum(현금기록) / len(현금기록)) if 현금기록 else 100.0,
                "빈날": 빈날수, "돈날": len(현금기록)}
    ...
    print("=" * 104)
    print("  143차 · 돈이 얼마나 놀고 있나")
    print("=" * 104)
    print(f"\n  {'무엇':<34}{'끝 자산':>15}{'낙폭':>8}{'산 것':>7}"
          f"{'해마다':>8}{'현금 비율':>10}{'빈손인 날':>10}")

    def 줄(라, c):
        r = 시뮬(c)
        해 = r["돈날"] / 245 if r.get("돈날") else 1
        print(f"  {라:<34}{r['끝']:>15,.0f}{r['낙']:>7.1f}%{r['산']:>7}"
              f"{r['산']/해:>8.1f}{r['현금비율']:>9.1f}%"
              f"{r['빈날']/max(r['돈날'],1)*100:>9.1f}%")
        return r

    기 = 줄("지금 (갭 -3.5)", 기본틀)

    print("\n  ── D 문턱을 낮추면 ──")
    for 갭 in (-3.0, -2.5, -2.0):
        c = dict(기본틀)
        c["상대갭"] = 갭
        줄(f"갭 {갭}", c)
    ...
    c = dict(기본틀)
    c["낙사다리"] = [(-10, -5), (999, -10)]
    줄("시장 -10%↓ 낙폭-5 / 그밖 -10", c)
    c2 = dict(기본틀)
    c2["갭사다리"] = [(-10, -2.0), (999, -3.5)]
    줄("시장 -10%↓ 갭-2.0 / 그밖 -3.5", c2)
    ...
    print("\n  ── 비중을 키우면 (같은 횟수, 더 많이) ──")
    for 비중값 in (0.25, 0.33, 0.50):
        c = dict(기본틀)
        c["비중"] = 비중값
        줄(f"한 종목에 {int(비중값*100)}%", c)

    print("\n" + "=" * 104)
    print("  읽는 법")
    print("    - **현금 비율**이 90%면 돈의 열에 아홉이 놀고 있다는 뜻이다")
    print("    - **빈손인 날**은 주식을 하나도 안 들고 있던 날의 비율이다")
    print("    - ⚠️ 122차: 자주 사면 **비용이 삼킨다** (10년 107번 vs 4,521번 = 42배)")
    print("      그러니 「많이 사는 게 좋다」가 아니라 **얼마나 노는지 알고 정하자**는 것이다")
    print("=" * 104)
