#!/usr/bin/env python3
r"""
andor_lab.py — **175차 · AND 말고 OR 로 하면** (2026-09-09 신설)

## 사용자 지적
```
「어제 범위를 좁히지 말고 테스트하자고 했는데, **볼린저 20일 or 60일도 같은 사례**인 것 같아.
  **다른 분야에서도 or가 아니라 and일 때 어떻게 달라지는지** 테스트해보면 좋을 것 같아」
```
맞다. 지금까지 시험은 **거의 전부 AND** 였다 —
「재무 **그리고** 크기 **그리고** 볼린저 **그리고** 낙폭 **그리고** 갭」.
조건을 더할수록 **기회가 줄어드는 구조**다.
`OR` 로 하면 반대다 — **기회가 늘어난다**

## ⚠️ 왜 이걸 한 번도 안 해봤나
```
「좋은 조건을 다 갖춘 것을 산다」가 당연해 보였다.
그런데 사용자 목표는 **매수 기회 포착**이다.
「하나라도 세게 걸린 것을 산다」가 더 맞을 수 있다 — 재본 적이 없다
```

## 재는 것
```
A 짝마다 AND vs OR      우리 재료 여섯을 둘씩 묶어 AND / OR 를 나란히
B 조건 개수             「여섯 중 N개 이상 걸린 것」 — N을 1~6으로
C 강한 OR              「하나라도 **아주 세게**(문턱을 두 배로) 걸린 것」
D 섞기                 「재무는 꼭(AND) + 나머지는 하나라도(OR)」
E 몇 개나 걸리나        후보들이 보통 몇 개 조건에 걸리나
```

## 잣대 — 155차와 같다
```
① 1년에 몇 개  ② 20일 뒤 오른 비율  ③ 평균 몇 %
바탕(아무 종목·아무 날) 20일 = **43.5% 오름 · +0.33%**
⚠️ **개수를 먼저 본다** — OR 는 개수를 늘리는 게 목적이다
```

쓰는 법:
    python scripts\andor_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    # ══ 재료 여섯 — 지금 규칙이 AND 로 묶고 있는 것들 ══
    def 있(x, k):
        return x.get(k) is not None

    조건들 = [
        ("재무", lambda x: x.get("재통과") == 1.0, "잉여금30·부채80·흑자"),
        ("크기", lambda x: 300 <= x["시총억"] < 2000, "시총 300~2,000억"),
        ("대금", lambda x: x["대금억"] >= 1.0, "거래대금 1억↑"),
        ("볼린저", lambda x: x["볼린저"] <= -1.0, "20일 볼린저 -1.0σ"),
        ("낙폭", lambda x: x["낙폭20"] <= -10.0, "20일 낙폭 -10%"),
        ("긴낙폭", lambda x: 있(x, "낙폭60") and x["낙폭60"] <= -20.0,
         "60일 낙폭 -20%"),
    ]
    센조건 = [
        ("재무", lambda x: (x.get("재통과") == 1.0
                           and 있(x, "잉여금") and x["잉여금"] >= 60)),
        ("크기", lambda x: 300 <= x["시총억"] < 1000),
        ("대금", lambda x: x["대금억"] >= 3.0),
        ("볼린저", lambda x: x["볼린저"] <= -1.5),
        ("낙폭", lambda x: x["낙폭20"] <= -20.0),
        ("긴낙폭", lambda x: 있(x, "낙폭60") and x["낙폭60"] <= -35.0),
    ]

    머2 = (f"  {'설정':<42}{'건수':>9}{'1년에':>7}"
    ...
    def 세기(라, fn, 최소=150):
        ...
        n = len(칸)
        if n < 최소:
            print(f"  {라:<42}{n:>9,}   표본 부족")
            return None
        줄 = f"  {라:<42}{n:>9,}{n/해수:>7.0f}"
        점 = None
        for 기 in 기간들:
    ...
    print("\n" + "=" * 122)
    print("  175차 · **AND 말고 OR 로 하면** — 조건을 더할수록 기회가 주는 구조를 뒤집는다")
    print(f"  후보 {len(사건):,}건 · 바탕(아무 종목·아무 날) 20일 43.5% · +0.33%")
    print(f"  ⚠️ 상장폐지는 {O.폐지손실:.0f}% 손실로 센다")
    print("  ⚠️ **개수를 먼저 본다** — OR 는 기회를 늘리는 게 목적이다")
    print("=" * 122)

    print("\n  ── 기준 ──")
    print(머2)
    지금 = 세기("지금 규칙 (여섯 중 다섯을 **전부** AND)",
                lambda x: all(f(x) for _, f, _ in 조건들[:5]))
    세기("아무 종목·아무 날", lambda x: True)

    # ── A 짝마다 AND vs OR ──
    print("\n  ── A **둘씩 묶어** AND 와 OR 를 나란히 ──")
    print(머2)
    for a in range(len(조건들)):
        for b in range(a + 1, len(조건들)):
            라1, f1, _ = 조건들[a]
            라2, f2, _ = 조건들[b]
            세기(f"{라1} **그리고** {라2}",
                 lambda x, p=f1, q=f2: p(x) and q(x))
            세기(f"{라1} **또는** {라2}",
                 lambda x, p=f1, q=f2: p(x) or q(x))
        print()

    # ── B 몇 개 이상 걸리면 ──
    print("\n  ── B **여섯 중 N개 이상** 걸린 것 ──")
    print("     (N=6 이 지금 방식에 가깝고, N=1 이 가장 느슨하다)")
    print(머2)
    for x in 사건:
        x["_몇개"] = sum(1 for _, f, _ in 조건들 if f(x))
    for N in (1, 2, 3, 4, 5, 6):
        세기(f"여섯 중 **{N}개 이상**",
             lambda x, n2=N: x["_몇개"] >= n2)

    # ── C 하나라도 아주 세게 ──
    print("\n  ── C **하나라도 아주 세게** 걸린 것 (문턱을 두 배로) ──")
    print(머2)
    for 라, f in 센조건:
        세기(f"**{라}만** 아주 세게", f)
    세기("여섯 중 **하나라도** 아주 세게",
         lambda x: any(f(x) for _, f in 센조건))
    for x in 사건:
        x["_센개"] = sum(1 for _, f in 센조건 if f(x))
    for N in (2, 3):
        세기(f"**{N}개 이상** 아주 세게", lambda x, n2=N: x["_센개"] >= n2)

    # ── D 섞기 ──
    print("\n  ── D **재무는 꼭(AND) · 나머지는 하나라도(OR)** ──")
    print(머2)
    재무f = 조건들[0][1]
    나머지 = 조건들[1:]
    세기("재무 + 나머지 다섯 **전부**",
         lambda x: 재무f(x) and all(f(x) for _, f, _ in 나머지))
    for N in (1, 2, 3, 4):
        세기(f"재무 + 나머지 중 **{N}개 이상**",
             lambda x, n2=N: (재무f(x)
                              and sum(1 for _, f, _ in 나머지 if f(x)) >= n2))
    세기("재무 + **하나라도 아주 세게**",
         lambda x: 재무f(x) and any(f(x) for _, f in 센조건[1:]))
    print("\n     (재무 대신 대금을 꼭 요구하면)")
    대금f = 조건들[2][1]
    for N in (1, 2, 3):
        세기(f"대금 + 나머지 중 **{N}개 이상**",
             lambda x, n2=N: (대금f(x) and sum(
                 1 for 라3, f, _ in 조건들 if 라3 != "대금" and f(x)) >= n2))

    # ── E 보통 몇 개나 걸리나 ──
    print("\n  ── E 후보들이 **보통 몇 개** 조건에 걸리나 ──")
    from collections import Counter as _C
    칸2 = _C(x["_몇개"] for x in 사건)
    총 = len(사건)
    print(f"  {'걸린 조건 수':<16}{'건수':>12}{'비율':>8}")
    for k in range(7):
        c = 칸2.get(k, 0)
        print(f"  {k}개{'':<13}{c:>12,}{c/총*100:>7.1f}%")

    print("\n" + "=" * 122)
    print("  읽는 법")
    print("    - ⚠️ **개수를 먼저 본다.** 이김%만 보면 AND 가 늘 이긴다 —")
    print("      조건을 더할수록 걸러지니 당연하다. 그건 **기회를 깎는 것**이다")
    print("    - B 에서 **N을 낮추면 개수가 몇 배 되는지**, 이김%는 얼마나 떨어지는지 본다")
    print("    - 「1년 개수 × 이김%」가 실제로 버는 힘에 가깝다")
    print("    - 좋아 보이면 **165차 4관문**으로 넘긴다")
    print("=" * 122)
    return 0
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      or "2026-09-09_175차_AND말고OR.txt")
