#!/usr/bin/env python3
r"""
when_lab.py — **142차 · 언제 사느냐 (매수 기회를 늘린다)** (2026-09-08 신설)

## 사용자 지시
```
「매수 타이밍·기회 포착이 중요하지 얼마나 사느냐는 다음 문제야.
  매수 타이밍이 있어야 얼마나 사느냐도 정할 수 있으니까」
```

## 지금 무엇이 문제인가
```
사는 날이 **3%뿐**이다 (36일에 한 번 · 10.4년에 103건)
후보 있는 날은 67%인데 **갭 문턱 -3.5%p 가 96%를 걸러낸다**
131차: 일하는 건 갭 문턱뿐 — 그 안에서 어떻게 고르든 무작위와 같다
```

## 안 해본 것 — **국면에 따라 문턱을 움직인다**
```
140차: 시장이 크게 빠진 날 산 것은 40일 뒤 **+24.4%**, 안 빠진 날은 **-0.4%**
129차: 그걸 **조건으로 얹었더니** 전패 (후보가 줄어서)
⇒ 얹지 말고 **문턱 자체를 국면에 따라 움직인다**
     시장이 빠진 날    -> 느슨하게 (기회를 늘린다)
     시장이 안 빠진 날  -> 엄하게   (안 산다)
```

## 재는 것
```
D 갭 문턱 고정값 다시   -5.0 ~ -1.5  (기회 vs 성적)
A 갭 문턱을 국면별로
B 낙폭 문턱을 국면별로
C 볼린저 문턱을 국면별로
E 셋 다
```
⚠️ **산 것 수(기회)를 반드시 같이 본다.** 돈만 보면 기회가 줄어도 좋아 보인다

쓰는 법:
    python scripts\when_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    기본틀["대금하한"] = 1.0
    기본틀["거름"] = lambda x: (
        x.get("잉여금") is None or True)   # 재무는 수집에서 이미 걸렸다

    낙칸 = [v for v in (낙폭("코스닥", i2) for i2 in range(260, len(날)))
            if v is not None]
    낙칸.sort()

    def 분위(p):
        return 낙칸[int(len(낙칸) * p)] if 낙칸 else 0

    print("=" * 100)
    print("  142차 · 언제 사느냐 — **매수 기회를 늘린다**")
    print("=" * 100)
    print(f"\n  코스닥 20일 낙폭 분포 ({len(낙칸):,}일)")
    print(f"    아래 5% {분위(0.05):>7.1f}%  10% {분위(0.10):>7.1f}%"
          f"  25% {분위(0.25):>7.1f}%  가운데 {분위(0.50):>7.1f}%"
          f"  위 25% {분위(0.75):>7.1f}%")
    print("\n  ⚠️ **산 것 수(기회)를 같이 본다.** 돈만 보면 기회가 줄어도 좋아 보인다")
    ...
    for 끝년, 라 in ((None, "전체 기간"), ("2024", "2025·26 제외")):
        ...
        print("\n    ── D 갭 문턱 고정값 (기회 vs 성적) ──")
        for 갭 in (-5.0, -4.5, -4.0, -3.5, -3.0, -2.5, -2.0, -1.5):
            c = dict(기본틀)
            c["상대갭"] = 갭
            표(시뮬(c, 끝년=끝년), f"갭 {갭}%p 고정", 기)

        print("\n    ── A 갭 문턱을 **국면에 따라** ──")
        for 이름, 사 in (
                ("시장 -10%↓ 갭-2.0 / 그밖 -3.5", [(-10, -2.0), (999, -3.5)]),
                ("시장 -5%↓ 갭-2.5 / 그밖 -3.5", [(-5, -2.5), (999, -3.5)]),
                ("-10%↓ -2.0 / -5%↓ -3.0 / 그밖 -4.5",
                 [(-10, -2.0), (-5, -3.0), (999, -4.5)]),
                ("-10%↓ -1.5 / -5%↓ -2.5 / 그밖 -5.0",
                 [(-10, -1.5), (-5, -2.5), (999, -5.0)]),
                ("(반대) -10%↓ -5.0 / 그밖 -2.5",
                 [(-10, -5.0), (999, -2.5)])):
            c = dict(기본틀)
            c["갭사다리"] = 사
        ...
        print("\n    ── B 낙폭 문턱을 국면에 따라 ──")
        for 이름, 사 in (("-10%↓ 낙폭-5 / 그밖 -10", [(-10, -5), (999, -10)]),
                         ("-10%↓ -5 / -5%↓ -8 / 그밖 -12",
                          [(-10, -5), (-5, -8), (999, -12)])):
            c = dict(기본틀)
            c["낙사다리"] = 사
        ...
        print("\n    ── C 볼린저 문턱을 국면에 따라 ──")
        for 이름, 사 in (("-10%↓ 볼-0.5 / 그밖 -1.0", [(-10, -0.5), (999, -1.0)]),
                         ("-10%↓ -0.5 / -5%↓ -0.8 / 그밖 -1.3",
                          [(-10, -0.5), (-5, -0.8), (999, -1.3)])):
            c = dict(기본틀)
            c["볼사다리"] = 사
            표(시뮬(c, 끝년=끝년), 이름, 기)
        ...
        print("\n    ── E 셋 다 국면에 따라 ──")
        c = dict(기본틀)
        c["갭사다리"] = [(-10, -2.0), (-5, -3.0), (999, -4.5)]
        c["낙사다리"] = [(-10, -5), (-5, -8), (999, -12)]
        c["볼사다리"] = [(-10, -0.5), (-5, -0.8), (999, -1.3)]
        표(시뮬(c, 끝년=끝년), "갭·낙폭·볼린저 전부 국면별", 기)
        print("")
