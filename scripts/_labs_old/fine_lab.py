#!/usr/bin/env python3
r"""
fine_lab.py — **186차 · 모든 재료의 눈금을 촘촘히** (2026-09-09 신설)
...
## 지금까지 얼마나 성겼나
```
볼린저   20일 · -1.0σ **한 점**만 쓴다   (157차가 창을 훑었지만 문턱은 4개뿐)
낙폭     20일 · -10% **한 점**
...
## 여기서 훑는 것 — **눈금을 잘게**
```
A 볼린저   창 5·10·20·40·60·120·250일  x  문턱 -0.5 ~ -2.5 (일곱)
B 낙폭     창 3·5·10·20·40·60·120·250일 x  문턱 -3 ~ -40% (여덟)
C 60일선 대비 · 회전율 · 변동성 · 가격대   각각 잘게
D 재무     잉여금 0~300 (여덟) · 부채 40~무제한 (일곱) · ROE · 영업이익률
E 거래대금  0.3 ~ 20억 (아홉) · 거래량
F 수급     창 1·3·5·10·20·60일 x 위/아래
G 공시·뉴스 **창을 넓힌다** — 그날 / 3일 / 5일 / 20일 안에 있었나
H 시장     시장 낙폭 창 5·10·20·60일 x 문턱
```
⚠️ **한 재료씩** 잰다 (조합은 163차가 이미 전수로 했다).
   여기서는 **어느 눈금이 가장 좋은지**만 찾는다

## 잣대
```
① 1년에 몇 개  ② 20일 뒤 오른 비율  ③ 평균 몇 %
바탕(아무 종목·아무 날) 20일 = 43.5% · +0.33%
⚠️ **① 이 먼저다** — 잘 맞혀도 기회가 없으면 못 쓴다
```

쓰는 법:
    python scripts\fine_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    for i, d1 in enumerate(날):
        ...
        for code, v in 주가[d1].items():
            ...
            # ── 186차: 눈금을 잘게 ──
            창값 = {}
            for w in (3, 5, 10, 20, 40, 60, 120, 250):
            ...
            for w in (5, 10, 20, 40, 60, 120, 250):
                if kk >= w:
                    ...
                    창값[f"볼{w}"] = (c1 - m2) / (2 * d2)
                    if w == 20:
                        창값["_1시그마"] = (d2 / c1 * 100) if c1 else None
            for w in (20, 60, 120):
                if kk >= w:
                    mm = st.mean(sq[kk - w + 1:kk + 1])
                    창값[f"선{w}대비"] = ((c1 / mm - 1) * 100) if mm else None
    ...
    자리표 = {d: i2 for i2, d in enumerate(날)}
    for x in 사건:
        s = 공날.get(x["code"]) or set()
        i2 = x["인"] - 1
        for w in (1, 3, 5, 20):
            앞 = 날[max(0, i2 - w + 1):i2 + 1]
            x[f"공시{w}"] = 1.0 if any(d in s for d in 앞) else 0.0

    def 바탕(x):
        return (x.get("재통과") == 1.0 and 300 <= x["시총억"] < 2000
                and x["대금억"] >= 1.0)

    def 세기(라, fn, 최소=150, 조용=False):
        칸 = [x for x in 사건 if fn(x)]
        n = len(칸)
        v = [z["_20"] for z in 칸 if z.get("_20") is not None]
        if n < 최소 or len(v) < 80:
            if not 조용:
                print(f"  {라:<30}{n:>9,}   표본 부족")
            return None
        이 = sum(1 for z in v if z > 0) / len(v) * 100
        평 = sum(v) / len(v)
        v40 = [z["_40"] for z in 칸 if z.get("_40") is not None]
        이4 = (sum(1 for z in v40 if z > 0) / len(v40) * 100) if v40 else 0
        평4 = (sum(v40) / len(v40)) if v40 else 0
        print(f"  {라:<30}{n:>9,}{n/해수:>8.0f}{이:>9.1f}%{평:>+9.2f}"
              f"{이4:>9.1f}%{평4:>+9.2f}")
        return (이, 평, n)

    머2 = (f"  {'설정':<30}{'건수':>9}{'1년에':>8}"
    ...
    print("\n" + "=" * 108)
    print("  186차 · **모든 재료의 눈금을 촘촘히**")
    print("  「볼린저뿐만 아니라 **가격이든 기간이든 세분화할 수 있는 거**」 (사용자)")
    print(f"  후보 {len(사건):,}건 · 바탕(아무 종목·아무 날) 20일 43.5% · +0.33%")
    print(f"  ⚠️ 상장폐지 {O.폐지손실:.0f}% · **재무·크기·대금은 깔고 간다**")
    print("=" * 108)
    print("\n  ── 기준 ──")
    print(머2)
    세기("아무 종목·아무 날", lambda x: True)
    세기("**지금 규칙** (볼20 -1.0 · 낙20 -10)",
         lambda x: 바탕(x) and x["볼린저"] <= -1.0 and x["낙폭20"] <= -10)

    print("\n  ── A **볼린저** 창 7 × 문턱 7 ──")
    print(머2)
    for w in (5, 10, 20, 40, 60, 120, 250):
        for 문 in (-0.5, -0.75, -1.0, -1.25, -1.5, -2.0, -2.5):
            세기(f"볼린저 {w}일 {문}σ",
                 lambda x, a=w, b=문: (바탕(x) and 있(x, f"볼{a}")
                                      and x[f"볼{a}"] <= b), 조용=True)
        print()

    print("\n  ── B **낙폭** 창 8 × 문턱 8 ──")
    print(머2)
    for w in (3, 5, 10, 20, 40, 60, 120, 250):
        for 문 in (-3, -5, -8, -10, -15, -20, -30, -40):
            세기(f"낙폭 {w}일 {문}%",
                 lambda x, a=w, b=문: (바탕(x) and 있(x, f"낙{a}")
                                      and x[f"낙{a}"] <= b), 조용=True)
        print()

    print("\n  ── C **이동평균선 대비 · 변동성 · 회전율 · 가격대** ──")
    print(머2)
    for w in (20, 60, 120):
        for 문 in (-5, -10, -20, -30):
            세기(f"{w}일선 대비 {문}%",
                 lambda x, a=w, b=문: (바탕(x) and 있(x, f"선{a}대비")
                                      and x[f"선{a}대비"] <= b), 조용=True)
        print()
    for 라, lo, hi in (("1σ 5% 미만(조용)", 0, 5), ("5~8%", 5, 8),
                       ("8~12%", 8, 12), ("12~18%", 12, 18),
                       ("18%↑(사나움)", 18, 9e9)):
        세기(f"그 종목 변동성 {라}",
             lambda x, a=lo, b=hi: (바탕(x) and 있(x, "_1시그마")
                                    and a <= x["_1시그마"] < b))
    print()
    for 라, lo, hi in (("0.3% 미만", 0, 0.3), ("0.3~1%", 0.3, 1),
                       ("1~3%", 1, 3), ("3~8%", 3, 8), ("8%↑", 8, 9e9)):
        세기(f"회전율 {라}",
             lambda x, a=lo, b=hi: 바탕(x) and a <= x.get("회전율", -1) < b)
    print()
    for 라, lo, hi in (("1,000원 미만", 0, 1000), ("1,000~2,000", 1000, 2000),
                       ("2,000~5,000", 2000, 5000),
                       ("5,000~1만", 5000, 10000),
                       ("1~2만", 10000, 20000), ("2~5만", 20000, 50000),
                       ("5만원↑", 50000, 9e9)):
        세기(f"주가 {라}",
             lambda x, a=lo, b=hi: 바탕(x) and a <= (x.get("원시") or 0) < b)

    print("\n  ── D **재무** 눈금 ──")
    print(머2)
    def 신호(x):
        return (300 <= x["시총억"] < 2000 and x["대금억"] >= 1.0
                and x["볼린저"] <= -1.0 and x["낙폭20"] <= -10)
    for 잉 in (0, 10, 20, 30, 50, 80, 150, 300):
        세기(f"잉여금 {잉}%↑",
             lambda x, a=잉: (신호(x) and 있(x, "잉여금") and x["잉여금"] >= a),
             조용=True)
    print()
    for 부 in (40, 60, 80, 100, 150, 200, 9999):
        세기(f"부채 {'안 봄' if 부 > 1000 else str(부) + '% 이하'}",
             lambda x, a=부: (신호(x) and 있(x, "부채") and x["부채"] <= a),
             조용=True)
    print()
    for 라, lo in (("ROE 0%↑", 0), ("5%↑", 5), ("10%↑", 10), ("15%↑", 15)):
        세기(f"{라}", lambda x, a=lo: 신호(x) and 있(x, "ROE") and x["ROE"] >= a)
    for 라, lo in (("영업이익률 0%↑", 0), ("5%↑", 5), ("10%↑", 10),
                   ("20%↑", 20)):
        세기(f"{라}", lambda x, a=lo: (신호(x) and 있(x, "영업이익률")
                                     and x["영업이익률"] >= a))
    ...
    def 신호2(x):
        return (x.get("재통과") == 1.0 and 300 <= x["시총억"] < 2000
                and x["볼린저"] <= -1.0 and x["낙폭20"] <= -10)
    for 대 in (0.1, 0.3, 0.5, 1, 2, 3, 5, 10, 20):
        세기(f"거래대금 {대}억↑",
             lambda x, a=대: 신호2(x) and x["대금억"] >= a, 조용=True)

    print("\n  ── F **수급** 창 (있는 것만) ──")
    print(머2)
    for k in ("외인1", "외인5", "외인20", "기관5", "기관20", "개인5"):
        vs = sorted(x[k] for x in 사건 if 있(x, k))
        if len(vs) < 1000:
            continue
        낮, 높 = vs[len(vs) // 5], vs[len(vs) * 4 // 5]
        세기(f"{k} 아래 20%",
             lambda x, a=k, b=낮: 바탕(x) and 있(x, a) and x[a] <= b)
        세기(f"{k} 위 20%",
             lambda x, a=k, b=높: 바탕(x) and 있(x, a) and x[a] >= b)

    print("\n  ── G **공시 창을 넓히면** (한 번도 안 해봤다) ──")
    print(머2)
    for w in (1, 3, 5, 20):
        세기(f"{w}일 안에 공시 있음",
             lambda x, a=w: 바탕(x) and x.get(f"공시{a}") == 1.0)
        세기(f"{w}일 안에 공시 없음",
             lambda x, a=w: 바탕(x) and x.get(f"공시{a}") == 0.0)

    print("\n  ── H **시장 낙폭** 눈금 ──")
    print(머2)
    for 문 in (-3, -5, -8, -10, -15, -20):
        세기(f"시장 20일 {문}%↓ 인 날",
             lambda x, a=문: (바탕(x) and 있(x, "시장낙폭")
                             and x["시장낙폭"] <= a))
    print()
    for 문 in (-15, -10, -5, 0, 5):
        세기(f"상대강도 {문}%p 이하 (시장보다 더 빠짐)",
             lambda x, a=문: (바탕(x) and 있(x, "상대강도")
                             and x["상대강도"] <= a))

    print("\n" + "=" * 108)
    print("  읽는 법")
    print("    - **1년에 몇 개**를 먼저 본다. 잘 맞혀도 기회가 없으면 못 쓴다")
    print("    - 지금 규칙보다 **개수도 많고 이김%도 높은 눈금**이 있으면 그게 답이다")
    print("    - 한 재료씩만 잰다 — 조합은 163차가 전수로 했다")
