#!/usr/bin/env python3
r"""
rise_lab.py — **140차 · 오르는 것을 무엇이 가르나** (2026-09-07 신설)

## 사용자 지시
```
「파는 것도 중요하지만 **상승 포인트·매수 타이밍**을 잘 잡는 게 더 중요하다.
  그걸 인간이 어려워하니까. 파는 거는 충분히 수익 났다고 생각하면 팔아도 된다」
```
⇒ 지금까지 **파는 쪽만 이겼다**(124차). 사는 쪽은 스물다섯 번 다 졌다.
  그런데 진 이유가 「신호가 없다」인지 **「신호를 안 찾아봤다」**인지 모른다

## 이 시험이 다른 점 — **자본 시뮬을 안 쓴다**
```
지금까지  신호 하나 -> 규칙에 넣어 -> 10.4년 자본 시뮬 -> 이겼나 졌나
          한 번에 하나씩. 20~30개가 한계였다
여기서    빠진 사건 하나하나에 **지표를 전부 붙이고**,
          지표마다 **5분위로 갈라 40일 수익률**을 본다
          -> 한 번에 **수십 개 지표**를 훑는다. 자본 시뮬은 나중에
```
⚠️ 여기서 나온 것은 **후보일 뿐**이다. 자본 시뮬과 4관문을 거쳐야 규칙이 된다.
   하지만 **어디를 파야 하는지**는 여기서 정해진다

## 재는 것
```
대상   20일 -10%↓ + 볼린저 -1.0↓ 인 사건 (= 우리가 사고 싶어 하는 상태)
지표   수급 7 · 재무 6 · 가격 8 · 섹터 3 · 시장 2 · 공시 2  = 약 28개
잣대   지표를 5분위로 갈라 **40일 수익률**의 중앙·평균·이김%
       위 20% 와 아래 20% 의 **차이**가 클수록 센 신호다
```

쓰는 법:
    python scripts\rise_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    print("  앞으로의 수익률 붙이는 중...", flush=True)
    for x in 사건:
        x["_20"] = 앞수익(x, 20)
        x["_40"] = 앞수익(x, 40)
    ...
    def 오분위(키, 기간="_40"):
        칸 = [(x[키], x[기간]) for x in 사건
              if x.get(키) is not None and x.get(기간) is not None]
        if len(칸) < 1000:
            return None
        ...
        for k in range(5):
            ...
            v = [z[1] for z in 조각]
            v.sort()
            묶음.append((len(v), v[len(v) // 2], sum(v) / len(v),
                         sum(1 for z in v if z > 0) / len(v) * 100))
        return 묶음

    print("\n" + "=" * 104)
    print("  140차 · 빠진 종목(20일 -10%↓ · 볼린저 -1.0↓) 중 **오르는 것을 무엇이 가르나**")
    print(f"  대상 {len(사건):,}건 · 40거래일 수익률 · 지표를 5분위로 갈랐다")
    print("=" * 104)
    전체 = [x["_40"] for x in 사건 if x.get("_40") is not None]
    전체.sort()
    바탕평균 = sum(전체) / len(전체) if 전체 else 0
    바탕이김 = (sum(1 for v in 전체 if v > 0) / len(전체) * 100) if 전체 else 0
    print(f"\n  바탕(전부): {len(전체):,}건 · 중앙 {전체[len(전체)//2]:.2f}% "
          f"· 평균 {바탕평균:.2f}% · 이김 {바탕이김:.1f}%")

    센것 = []
    for 이름, 키 in 지표들:
        if 키 is None:
            print("\n  " + 이름)
            print(f"  {'지표':<24}{'맨아래20%':>22}{'맨위20%':>22}{'차이':>10}")
            print(f"  {'':<24}{'평균':>11}{'이김%':>11}"
                  f"{'평균':>11}{'이김%':>11}{'':>10}")
            continue
        묶음 = 오분위(키)
        if not 묶음:
        ...
        위 = 묶음[4]
        차 = 위[2] - 아[2]
        print(f"  {이름:<24}{아[2]:>11.2f}{아[3]:>11.1f}"
              f"{위[2]:>11.2f}{위[3]:>11.1f}{차:>10.2f}")
        센것.append((abs(차), 이름, 키, 아, 위, 차))

    센것.sort(key=lambda z: -z[0])
    print("\n" + "=" * 104)
    print("  ⭐ 차이가 큰 순 (위 20% 평균 - 아래 20% 평균)")
    print("=" * 104)
    print(f"  {'지표':<26}{'아래20% 평균':>14}{'위20% 평균':>14}"
          f"{'차이':>10}{'어느 쪽이 좋나':>16}")
    for _, 이름, 키, 아, 위, 차 in 센것[:14]:
        쪽 = "**높을수록**" if 차 > 0 else "**낮을수록**"
        print(f"  {이름:<26}{아[2]:>14.2f}{위[2]:>14.2f}{차:>10.2f}{쪽:>16}")

    print("\n" + "=" * 104)
    print("  읽는 법")
    print("    - 차이가 0 근처면 그 지표는 **오르는 것을 못 가른다**")
    print("    - 차이가 크면 **후보**다. 자본 시뮬 + 4관문을 거쳐야 규칙이 된다")
    print("    - ⚠️ 5분위는 **사후에 나눈 것**이라 실전 문턱이 아니다.")
    print("      실전에서는 그날 그 값을 알 수 있어야 한다 (전부 전날까지의 값이다)")
    print("=" * 104)
