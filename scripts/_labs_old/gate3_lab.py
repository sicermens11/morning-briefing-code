#!/usr/bin/env python3
r"""
gate3_lab.py — **「낙폭 깊은 순으로 고르기」를 관문에 건다** (2026-09-07 신설)

## 무엇을 검증하나
```
지금        08:50에 갭이 제일 많이 벌어진 4종목을 산다
바꿈 후보   문턱(-3.5%p)을 넘은 것 중 **낙폭이 제일 깊은** 4종목을 산다
```
고르는 잣대 시험 결과 — **유일하게 두 기간 다 이겼다**:
```
                 전체 기간              2025·26 제외
갭 큰 순 (지금)   6,432만 · 낙폭 -3.3%   4,121만 · 낙폭 -3.3%
낙폭 깊은 순      6,704만 (+4%) -3.3%   4,250만 (+3%) -3.3%
                 산 것 103건 (같음)     76건 (같음)
볼린저 낮은 순    +6%                   **-2%**  ← 방향이 갈린다. 못 믿는다
```
⚠️ **+4%는 작다.** 우연일 수 있으므로 관문을 건다

## 관문
```
A 앞뒤 분할     시작년을 2015·2019·2021로 옮겨도 이기나
B 연도별        해마다 이기나 (몇 승 몇 패)
C ⭐ 무작위 대조  **아무렇게나 4개 고르기**를 200번 돌려 분포를 만든다.
                낙폭순이 그 분포의 **상위 25%** 안에 들어야 한다
D 오차 시뮬     08:50 예상체결가 오차 ±0.3 / 0.5 / 1.0%p 에서도 이기나
```
⚠️ 넷 다 통과해야 실전 규칙을 바꾼다. 하나라도 걸리면 안 바꾼다

쓰는 법:
    python scripts\gate3_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    def 낙폭순(x):
        return x["낙폭20"]           # 음수라 오름차순 = 깊은 순

    도전 = dict(기본틀)
    도전["고르기"] = 낙폭순

    def 끝자산(r):
        # 시뮬은 {"끝","연","낙","산"} 딕셔너리를 준다
        return (r or {}).get("끝") or 0.0

    # ── A 앞뒤 분할 ──
    print("=" * 96)
    print("  A 앞뒤 분할 — 시작 시점을 옮겨도 이기나")
    print("=" * 96)
    print(머)
    ...
    print("=" * 96)
    이김, 짐 = 0, 0
    print(f"  {'해':<8}{'갭 큰 순':>16}{'낙폭 깊은 순':>18}{'차이':>10}")
    for y in range(2011, 2027):
        a = 시뮬(기본틀, 시작년=str(y), 끝년=str(y))
        b = 시뮬(도전, 시작년=str(y), 끝년=str(y))
        av, bv = 끝자산(a), 끝자산(b)
        if not av or not bv:
            continue
        차 = (bv / av - 1) * 100 if av else 0
        if 차 > 0:
            이김 += 1
        elif 차 < 0:
            짐 += 1
        print(f"  {y:<8}{av:>16,.0f}{bv:>18,.0f}{차:>9.1f}%")
    print(f"\n  ⇒ **{이김}승 {짐}패**")

    # ── C 무작위 대조 ──
    print("\n" + "=" * 96)
    print("  C ⭐ 무작위 대조 — 아무렇게나 4개 고르면?")
    print("=" * 96)
    번수 = 200
    분포 = []
    for s in range(번수):
        _r = _rnd.Random(1000 + s)
        c2 = dict(기본틀)
        c2["고르기"] = lambda x, _r=_r: _r.random()
        v = 끝자산(시뮬(c2))
        if v:
            분포.append(v)
        if (s + 1) % 50 == 0:
            print(f"    {s+1}/{번수} 돌림", flush=True)
    분포.sort()
    나 = 끝자산(시뮬(도전))
    기본값 = 끝자산(시뮬(기본틀))
    위 = sum(1 for v in 분포 if v < 나)
    위기본 = sum(1 for v in 분포 if v < 기본값)
    print(f"\n  무작위 {len(분포)}번 — 가장 나쁨 {분포[0]:,.0f} · "
          f"중앙 {분포[len(분포)//2]:,.0f} · 가장 좋음 {분포[-1]:,.0f}")
    print(f"  갭 큰 순 (지금)  {기본값:>14,.0f}  → 상위 "
          f"{(1-위기본/len(분포))*100:.0f}%")
    print(f"  낙폭 깊은 순     {나:>14,.0f}  → 상위 "
          f"{(1-위/len(분포))*100:.0f}%")
    통과C = (1 - 위 / len(분포)) <= 0.25
    print(f"  ⇒ 상위 25% 안에 {'**든다** ✅' if 통과C else '못 든다 ❌'}")

    # ── D 오차 시뮬 ──
    print("\n" + "=" * 96)
    print("  D 오차 시뮬 — 08:50 예상체결가가 틀릴 때도 이기나")
    print("=" * 96)
    print(머)
    for 오차 in (0.0, 0.3, 0.5, 1.0):
        기 = 시뮬(기본틀, 오차=오차)
        표(기, f"오차 ±{오차}%p · 갭 큰 순")
        표(시뮬(도전, 오차=오차), f"오차 ±{오차}%p · **낙폭 깊은 순**", 기)
        print("")
