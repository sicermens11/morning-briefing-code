#!/usr/bin/env python3
r"""
hit_lab.py — **155차 · 매수 타이밍만 본다** (2026-09-08 신설)

## ⚠️⚠️ 나는 어제 지시를 반쪽만 적었다
```
09-07 사용자: 「자산은 내가 알아서 판단해 매수할 테니,
              **상승 기회·매수 기회 포착 관점에서만** 종목 추천해주면 된다」
내가 적은 것:  「비중·배분·손실 금액을 **브리핑에서** 말하지 않는다.
              시뮬에서 자본 시뮬을 쓰는 것은 **규칙끼리 견주는 잣대로만 쓴다**」
```
**마지막 줄이 빠져나갈 구멍이었다.** 그래서 오늘 하루 종일
자본 시뮬(자산·낙폭·매도가 다 들어간 것)로 판단하면서 지시를 지킨다고 여겼다.
09-08 에 같은 지적을 다시 받고서야 알았다

## 그래서 여기서는 **자본 시뮬을 안 쓴다**
```
잣대      ① 1년에 몇 개 나오나   (기회)
          ② 20·40일 뒤에 **오른 비율**  (이길 확률)
          ③ 평균 몇 % 올랐나
안 쓰는 것  낙폭 · 자산 · 비중 · 매도 규칙 · 돈÷낙폭
```
매도는 나중 문제다. **먼저 「자주 나오면서 잘 맞는」 신호를 찾는다**

## 왜 이게 핵심인가
```
우리 후보 (재무·시총·볼린저·낙폭 통과)   하루 평균 5.5개 · 1년 **약 1,400개**
그중 갭까지 통과해 사는 것              1년 **10개**
⇒ 갭이 **1,390개를 버린다**
```
그 1,390개가 정말 못 오르는 것들인가? **한 번도 안 세봤다**

## 재는 것
```
A 조건을 하나씩 빼면   갭 없이 · 볼린저 없이 · 낙폭 없이 · 재무 없이
                    -> 개수와 이길 확률이 어떻게 바뀌나
B 갭 문턱별          -3.5 / -2.5 / -1.5 / 없음
C 조건 조합          「자주 나오면서 이길 확률이 반은 넘는」 것을 찾는다
D 견줌              아무 종목이나 아무 날에나 샀을 때 (바탕)
```

쓰는 법:
    python scripts\hit_lab.py
"""
import glob
import io
import json
...
def main():
    ...
    def 재기(라, fn):
        ...
        n = len(칸)
        if n < 100:
            print(f"  {라:<40}{n:>8,}{'표본 부족':>50}")
            return
        줄 = f"  {라:<40}{n:>8,}{n/해수:>9.0f}"
        for 기 in 기간들:
    ...
    # 조건 조각들
    재무 = lambda x: x.get("재통과") == 1.0
    크기 = lambda x: 500 <= x["시총억"] < 2000
    대금 = lambda x: x["대금억"] >= 1.0
    볼1 = lambda x: 있(x, "볼린저") and x["볼린저"] <= -1.0
    낙10 = lambda x: 있(x, "낙폭20") and x["낙폭20"] <= -10
    갭 = lambda t: (lambda x: 있(x, "갭") and x["갭"] <= t)

    def 그리고(*fs):
        return lambda x: all(f(x) for f in fs)

    print("\n" + "=" * 112)
    print("  155차 · 매수 타이밍만 본다  (자본 시뮬·낙폭·매도 안 씀)")
    print(f"  후보 {len(사건):,}건 · 10.4년")
    print("=" * 112)
    print(f"\n  {'설정':<40}{'건수':>8}{'1년에':>9}"
          f"{'5일 이김':>8}{'평균':>8}{'20일 이김':>8}{'평균':>8}"
          f"{'40일 이김':>8}{'평균':>8}")
    print("  " + "-" * 108)

    재기("바탕 — 아무 종목·아무 날", lambda x: True)
    print("")
    재기("① 재무만", 재무)
    재기("② 재무+크기", 그리고(재무, 크기))
    재기("③ 재무+크기+거래대금", 그리고(재무, 크기, 대금))
    재기("④ ③+볼린저 -1.0", 그리고(재무, 크기, 대금, 볼1))
    재기("⑤ ④+낙폭 -10%  ← **지금 후보**", 그리고(재무, 크기, 대금, 볼1, 낙10))
    print("")
    재기("⑥ ⑤+갭 -1.5", 그리고(재무, 크기, 대금, 볼1, 낙10, 갭(-1.5)))
    재기("⑦ ⑤+갭 -2.5", 그리고(재무, 크기, 대금, 볼1, 낙10, 갭(-2.5)))
    재기("⑧ ⑤+갭 -3.5  ← **지금 사는 것**",
         그리고(재무, 크기, 대금, 볼1, 낙10, 갭(-3.5)))
    print("")
    print("  ── 조건을 하나씩 **빼면** (지금 후보 ⑤ 기준) ──")
    재기("갭 빼기 = ⑤ 그대로", 그리고(재무, 크기, 대금, 볼1, 낙10))
    재기("볼린저 빼기", 그리고(재무, 크기, 대금, 낙10))
    재기("낙폭 빼기", 그리고(재무, 크기, 대금, 볼1))
    재기("재무 빼기", 그리고(크기, 대금, 볼1, 낙10))
    재기("크기 빼기", 그리고(재무, 대금, 볼1, 낙10))
    print("")
    print("  ── 문턱을 느슨하게 (기회를 늘리려면) ──")
    for 볼t in (-0.5, -1.0, -1.5):
        for 낙t in (-5, -10, -15):
            재기(f"볼린저 {볼t} · 낙폭 {낙t}%",
                 그리고(재무, 크기, 대금,
                        lambda x, b=볼t: 있(x, "볼린저") and x["볼린저"] <= b,
                        lambda x, n=낙t: 있(x, "낙폭20") and x["낙폭20"] <= n))

    print("\n" + "=" * 112)
    print("  읽는 법")
    print("    - **1년에 몇 개**가 매수 기회의 실체다")
    print("    - **이김%** 가 바탕보다 높아야 그 조건이 일을 하는 것이다")
    print("    - 낙폭·자산·매도는 **여기서 안 본다**. 그건 사시는 분이 정할 몫이다")
    print("=" * 112)
    return 0
