#!/usr/bin/env python3
r"""
size_grid_lab.py — **134차 · 규모별로 문턱을 따로 찾는다** (2026-09-07 신설)

## 왜 다시 하나
```
118차 중형주 · 119차 대형주 — 둘 다 기각했다.
그런데 그때 한 것은 **소형주에서 찾은 문턱(볼린저 -1.0 · 낙폭 -10% · 갭 -3.5)을
대형주에 그대로 갖다 댄 것**이다.
AGENDA 에도 이렇게 적어 놨었다:
...
## 짜임새
```
규모   소형 500~2,000억 · 중형 2,000억~1조 · 대형 1조 이상
격자   볼린저 -2.0/-1.5/-1.0/-0.5  ×  20일낙폭 -20/-15/-10/-5  ×  갭 -5/-3.5/-2/0
       = 규모마다 64칸, 셋이면 **192칸**
고정   재무(잉여금30·부채80·흑자) · 거래대금 1억↑ · 하루 4종목 · 자산 20%
매도   124차에서 채택한 나눠팔기 (반 +15%/40일 · 반 +40%/90일)
```
⚠️ 재무를 고정하는 근거: 133차에서 재무를 얹자 **+185%**가 됐다.
   지금까지 검증된 신호 중 가장 힘이 세다

⚠️ 격자에서 1등이 나와도 **그게 답이라는 뜻이 아니다.** 격자는 과적합의 온상이다.
   1등은 **4관문에 걸어야** 쓴다 (110차에서 AutoSearch 순위표가 못 쓸 것으로 판명났다)

쓰는 법:
    python scripts\size_grid_lab.py
"""
import glob
import io
import json
...
def main():
        ...
        for 볼 in 볼들:
            for 낙 in 낙들:
                for 갭 in 갭들:
                    ...
                    r = 시뮬(c)
                    if r["산"] < 30:        # 표본이 너무 적으면 버린다
                        continue
                    점 = r["연"] / max(abs(r["낙"]), 3.0)
                    칸들.append((점, 볼, 낙, 갭, r))
        칸들.sort(key=lambda z: -z[0])
        for 점, 볼, 낙, 갭, r in 칸들[:8]:
            print(f"  {볼:>7.1f}{낙:>7.0f}{갭:>7.1f}"
    ...
    # ── 1등들을 2025·26 제외로 다시 ──
    print("=" * 96)
    print("  1등을 **2025·26 제외**로 다시 — 앞뒤가 같은 방향인가")
    print("=" * 96)
    print(머)
    for 라, 하, 상 in 규모들:
        if 라 not in 최고:
        ...
        c["볼린저"], c["낙폭20"], c["상대갭"] = 볼, 낙, 갭
        표(r, f"[{라}] 전체 기간")
        표(시뮬(c, 끝년="2024"), f"[{라}] 2025·26 제외")
        print("")

    # ── 지금 쓰는 규칙과 견줌 ──
    print("=" * 96)
    print("  지금 쓰는 규칙 (소형 · 볼린저 -1.0 · 낙폭 -10 · 갭 -3.5)")
    print("=" * 96)
    print(머)
    지금 = dict(기본틀)
    지금["시총하한"], 지금["시총상한"] = 500, 2000
    지금["볼린저"], 지금["낙폭20"], 지금["상대갭"] = -1.0, -10, -3.5
    표(시뮬(지금), "지금 규칙 · 전체 기간")
    표(시뮬(지금, 끝년="2024"), "지금 규칙 · 2025·26 제외")
    print("")
