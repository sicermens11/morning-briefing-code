#!/usr/bin/env python3
r"""
pick_lab2.py — **160차 · 후보 수와 갭 기준** (2026-09-08 신설)

## 왜 — 아침에 센 「좁게 잡은 여덟 개」 중 둘이다
```
후보 40개    왜 40인지 **근거가 없다.** 20·60·80·전부는 한 번도 안 재봤다
갭 기준      「후보들 갭의 **중앙값**」인데, 평균·전체종목중앙값은 안 재봤다
```
사용자: 「범위 좁힌 것도 테스트에 반영하고!」

## 잣대 — 155차와 같다 (자본 시뮬 안 씀)
```
① 1년에 몇 개 나오나   ② 20일 뒤 **오른 비율**   ③ 평균 몇 %
바탕(아무 종목·아무 날) 20일 = **44.7% 오름 · +0.65%**
```

## 재는 것
```
A 후보 수      10 · 20 · 40 · 60 · 80 · 120 · 전부   x 고르는 기준 4가지
B 갭 기준      후보중앙값 · 후보평균 · 전체종목중앙값 · 전체평균 · 시총가중
C 상대갭 문턱   -1.5 ~ -6.0  (기준마다 알맞은 문턱이 다르다)
D 하루 상한    1 · 2 · 4 · 8 · 무제한  (같은 날 여러 개 뜰 때)
```
⚠️ 후보 수를 바꾸면 **고르는 기준도 같이 봐야 한다** —
   40개를 「낙폭 깊은 순」으로 자르는 것과 「볼린저 낮은 순」은 다른 일이다

쓰는 법:
    python scripts\pick_lab2.py
"""
import glob
import io
import json
...
def main():
    ...
    def 바탕(x):
        return (재무통과(x) and 크기통과(x) and 대금통과(x)
                and 볼통과(x) and 낙통과(x))

    머2 = (f"  {'설정':<38}{'건수':>8}{'1년에':>7}"
    ...
    def 세기(라, 칸, 최소=150):
        n = len(칸)
        if n < 최소:
            print(f"  {라:<38}{n:>8,}   표본 부족")
            return None
        줄 = f"  {라:<38}{n:>8,}{n/해수:>7.0f}"
        점 = None
        for 기 in 기간들:
            ...
            if len(v) < 80:
                줄 += f"{'-':>9}{'-':>8}"
                continue
            이김 = sum(1 for z in v if z > 0) / len(v) * 100
            평균 = sum(v) / len(v)
            줄 += f"{이김:>8.1f}%{평균:>+8.2f}"
            if 기 == 20:
                점 = (이김, 평균, n)
    ...
    print("\n" + "=" * 118)
    print("  160차 · **후보 수**와 **갭 기준** — 근거 없이 좁혀놨던 둘")
    ...
    for x in 사건:
        if 바탕(x):
            묶.setdefault(x["인"], []).append(x)
    print(f"  조건 통과 {sum(len(v) for v in 묶.values()):,}건 · {len(묶):,}일"
          f" (하루 평균 {sum(len(v) for v in 묶.values())/max(len(묶),1):.1f}개)")

    # ── A 후보 수 x 고르는 기준 ──
    print("\n  ── A **후보 수**를 바꾸면 (상대갭 조건 없이) ──")
    print(머2)
    기준들 = (("낙폭 깊은 순", lambda x: x["낙폭20"]),
              ("볼린저 낮은 순", lambda x: x["볼린저"]),
              ("거래대금 큰 순", lambda x: -x["대금억"]),
              ("시총 작은 순", lambda x: x["시총억"]))
    for 기라, 키 in 기준들:
        for N in (10, 20, 40, 60, 80, 120, 10 ** 9):
            칸 = []
            for i2, v in 묶.items():
                칸 += sorted(v, key=키)[:N]
            세기(f"{기라} 상위 {'전부' if N > 10 ** 8 else N}개", 칸)
        print()

    # ── B 갭 기준 ──
    print("\n  ── B **갭 기준**을 바꾸면 (상대갭 -3.5%p 고정) ──")
    print(머2)

    def 갭기준들(i2, 후보):
        """그날의 「가운데 갭」을 여러 방식으로 낸다"""
        하루전부 = 갭표.get(날[i2]) or {}
        전부값 = [g for g in 하루전부.values() if g is not None]
        후값 = [x["갭"] for x in 후보 if x.get("갭") is not None]
        시총가중 = None
        if 하루전부:
            분자 = 분모 = 0.0
            for c3, g3 in 하루전부.items():
                v3 = 주가[날[i2]].get(c3)
                if v3 and g3 is not None:
                    분자 += g3 * v3[1]
                    분모 += v3[1]
            시총가중 = 분자 / 분모 if 분모 > 0 else None
        return {
            "후보중앙값 (지금)": st.median(후값) if 후값 else None,
            "후보평균": (sum(후값) / len(후값)) if 후값 else None,
            "전체종목 중앙값": st.median(전부값) if 전부값 else None,
            "전체종목 평균": (sum(전부값) / len(전부값)) if 전부값 else None,
            "시총가중 평균": 시총가중,
        }

    print("  (그날 후보 40개를 낙폭 깊은 순으로 고른 뒤 상대갭을 잰다)")
    기준칸 = {}
    for i2, v in 묶.items():
        후보 = sorted(v, key=lambda x: x["낙폭20"])[:40]
        중들 = 갭기준들(i2, 후보)
        for 라, 중 in 중들.items():
            if 중 is None:
                continue
            for x in 후보:
                if x.get("갭") is None:
                    continue
                기준칸.setdefault(라, []).append((x, x["갭"] - 중))
    for 라 in ("후보중앙값 (지금)", "후보평균", "전체종목 중앙값",
               "전체종목 평균", "시총가중 평균"):
        칸 = [x for x, 상 in 기준칸.get(라, []) if 상 <= -3.5]
        세기(f"{라}", 칸)

    # ── C 상대갭 문턱 (기준마다) ──
    print("\n  ── C **상대갭 문턱**을 바꾸면 (기준마다 알맞은 값이 다르다) ──")
    for 라 in ("후보중앙값 (지금)", "전체종목 중앙값", "시총가중 평균"):
        print(f"\n   [{라}]")
        print(머2)
        for 문 in (-1.5, -2.0, -2.5, -3.0, -3.5, -4.5, -6.0):
            칸 = [x for x, 상 in 기준칸.get(라, []) if 상 <= 문]
            세기(f"상대갭 {문}%p 이하", 칸)

    # ── D 하루 상한 ──
    print("\n  ── D **하루에 몇 개까지** 살까 (상대갭 -3.5%p · 후보중앙값) ──")
    print(머2)
    날별 = {}
    for x, 상 in 기준칸.get("후보중앙값 (지금)", []):
        if 상 <= -3.5:
            날별.setdefault(x["인"], []).append((x, 상))
    for 상한 in (1, 2, 4, 8, 10 ** 9):
        칸 = []
        for i2, v in 날별.items():
            칸 += [x for x, _ in sorted(v, key=lambda z: z[1])[:상한]]
        세기(f"하루 {'무제한' if 상한 > 10 ** 8 else str(상한) + '개'}까지", 칸)

    print("\n" + "=" * 118)
    print("  읽는 법")
    print("    - 바탕은 20일 **44.7% 오름 · +0.65%** 다. 이보다 높아야 일을 한 것이다")
    print("    - **1년에 몇 개**와 **이김%**를 같이 본다 — 하나만 좋으면 못 쓴다")
    print("    - A 에서 후보 수를 늘려도 이김%가 안 떨어지면 **40은 근거 없이 좁은 것**이다")
    print("    - B 에서 지금 기준(후보중앙값)보다 나은 게 있으면 4관문으로 넘긴다")
    print("=" * 118)
    return 0
...
if __name__ == "__main__":
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                      ...
                      os.environ.get("LAB_OUT") or "2026-09-08_160차_후보수와갭기준.txt")
