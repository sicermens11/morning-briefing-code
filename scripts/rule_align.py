...
def 산출물검사():
    ...
    민 = html.replace(",", "")
    잴것 = [
        ("화면 · 시총 하한", R.시총하한억, _수(민, r"시가총액 <b>(\d+)억~")),
        ("화면 · 시총 상한", R.시총상한억,
         _수(민, r"시가총액 <b>\d+억~(\d+)억</b>")),
        ("화면 · 볼린저", R.볼린저문턱,
         _수(html, r"볼린저 −([\d.]+)σ\)에 있다", lambda v: -v)),
        ("화면 · 20일 낙폭", R.낙폭20문턱,
         _수(html, r"20거래일 동안 <b>([\d.]+)% 넘게</b>", lambda v: -v)),
        ("화면 · 섹터 업종 수", float(len(R.섹터규칙)),
         _수(html, r"등 <b>(\d+)개 업종</b>")),
        ("화면 · 지수 20일", R.지수낙20문턱,
         _수(html, r"\(20일 −([\d.]+)% · 60일", lambda v: -v)),
        ("화면 · 지수 60일", R.지수낙60문턱,
         _수(html, r"· 60일 −([\d.]+)%\)에는", lambda v: -v)),
        ("화면 · 상대갭", R.상대갭문턱,
         _수(html, r"<b>([\d.]+)%p 더 빠진 것</b>", lambda v: -v)),
        ("화면 · 하루 최대", float(R.하루최대종목),
         _수(html, r"<b>최대 (\d+)종목</b>")),
        ("화면 · 앞 몫 목표", R.앞몫목표,
         _수(html, r"(?<!나머지 )반은 (?:<b>)?\+([\d.]+)%")),
        ("화면 · 뒤 몫 목표", R.뒷몫목표,
         _수(html, r"나머지 반은 (?:<b>)?\+([\d.]+)%")),
        ("화면 · 앞 몫 기한", float(R.앞몫기한),
         _수(html, r"앞의 반은 <b>(\d+)거래일</b>")),
        ("화면 · 뒤 몫 기한", float(R.뒷몫기한),
         _수(html, r"뒤의 반은 <b>(\d+)거래일</b>")),
    ]
    try:
