...
def main():
    ...
    def 재기(라, 사는자리, 시작="00000000", 끝="99999999"):
        """사는자리(x) -> 산 날 자리 또는 None. `시작`~`끝` 사건만 센다"""
        벌 = {n: [] for n in _기간들}
        산것 = 0
        늦음 = []
        _대상 = [x for x in 사건 if 시작 <= 날[x["i"]] < 끝]
        _해 = max(len([d for d in 날
                       if max(시작, _시작) <= d < 끝]) / 245, 0.1)
        for x in _대상:
            j = 사는자리(x)
            if j is None:
                continue
    ...
    def 분할재기(라, 갈래, 값, 시작="00000000", 끝="99999999"):
        벌 = {n: [] for n in _기간들}
        산것 = 마저 = 0
        단가차 = []
        for x in [z for z in 사건 if 시작 <= 날[z["i"]] < 끝]:
            첫 = x["i"] + 1
            if 첫 >= len(날):
                continue
    ...
    # ══ D ⭐⭐⭐ **앞뒤 분할** (4관문 ①) ══════════════════════
    #    위에서 좋아 보인 것이 **한 국면짜리**인지 가린다.
    #    R3판에서 어젯밤 상위 5개 중 **4개가 「앞 0건」**이었다 — 같은 함정을 피한다
    _반9 = 사건[len(사건) // 2]
    _가9 = 날[_반9["i"]]
    print("\n" + "=" * 112)
    print(f"  ⭐⭐⭐ **앞뒤 분할** — 앞 {_시작[:6]}~{_가9[:6]} · 뒤 {_가9[:6]}~")
    print("     ⚠️ **앞뒤 둘 다** 지금보다 나아야 믿는다. 한쪽만이면 한 국면짜리다")
    print("=" * 112)
    for 제목9, 갈9 in (("기술 신호", ("볼린저재진입", "양봉+거래량")),
                       ("⭐ 공시", ("챙길공시", "장중공시"))):
        print(f"\n  ── {제목9} ──")
        for 쪽9, s9, e9 in (("앞", _시작, _가9), ("뒤", _가9, "99999999")):
            print(f"   [{쪽9}]")
            print(머)
            재기("  [견줌] 지금", lambda x: x["i"] + 1, s9, e9)
            for 방9 in 갈9:
                재기("  " + 방9, _사는자리만들기(방9), s9, e9)
    print("\n  ── ⭐ 분할 매수 (가격 -5%) ──")
    _머분 = (f"  {'설정':<26}{'산 것':>9}{'마저삼':>8}"
             f"{'5일 이김':>9}{'평균':>9}{'20일 이김':>9}{'평균':>9}"
             f"{'40일 이김':>9}{'평균':>9}{'단가차':>9}")
    for 쪽9, s9, e9 in (("앞", _시작, _가9), ("뒤", _가9, "99999999")):
        print(f"   [{쪽9}]")
        print(_머분)
        분할재기("  [견줌] 지금", None, None, s9, e9)
        분할재기("  가격 50:50 · -5%", "가격", -5, s9, e9)
