# ==============================================================
#  queue_pairs7.ps1 — **ECOS 재료 둘을 gate7 에 붙인 뒤 조합 재판정** (2026-09-16 13:55)
#  pairs3 가 「못 만드는 재료」로 건너뛴 60일선대비↓+기준금리20↑(+54%) · 기준금리20↑+무역수지비↑(+26%) 를 잰다
#
...
#  SIZE_HI 없음 (지금 기준선 그대로). 앞줄(queue_vanish) 뒤 · 07:20~09:10 은 시작 안 함
# ==============================================================
...
$log = "run-logs\queue_pairs7_$(Get-Date -f yyyyMMdd_HHmm).log"
function 적기($s) {
...
function 앞줄끝났나 {
    $l = Get-ChildItem "run-logs\queue_vanish_*.log" -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime | Select-Object -Last 1
    if (-not $l) { return $true }
    return ((Get-Content $l.FullName -Raw -Encoding UTF8) -match 'queue_vanish 끝')
}

적기 "[0] 앞줄(queue_vanish)이 끝나길 기다린다"
$분 = 0
...
적기 "[PAIRS7] 13쌍을 Ⓗ 위에 얹어 4관문 + 기존 OR 공시 - 시작"
$env:BASE_GAP = "표본만+실전표본"
$env:BASE_RELGAP = "-3.5"
$env:BASE_SELL = "0.4,15,40 / 0.6,40,90"
$env:BASE_PICKS = "120"
$env:SIZE_HI = "999999"
$env:LAB_OUT = "2026-09-16_PAIRS7_실전기준선.txt"
...
$밖 = Join-Path "data\_labs" "2026-09-16_PAIRS7_실전기준선.txt"
